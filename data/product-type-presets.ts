export type ProductTypePresetSeed = {
  key: string;
  name: string;
  category: string;
  isSaleable: boolean;
...
export const productTypePresets: ProductTypePresetSeed[] = [
...
const exactNameToKey = new Map(productTypePresets.map((item) => [item.name, item.key]));

export function findProductTypePresetKey(name?: string | null, category?: string | null) {
  const normalized = String(name || '').trim();
  if (!normalized) return null;
  const exact = exactNameToKey.get(normalized);
  if (exact) return exact;
  if (normalized.includes('高层')) return 'highRiseResidential';
  if (normalized.includes('小高')) return 'midRiseResidential';
  if (normalized.includes('洋房')) return 'gardenHouse';
  if (normalized.includes('别墅') || normalized.includes('合院')) return 'villaCourtyard';
  if (normalized.includes('底商') || normalized.includes('商铺')) return 'streetRetail';
  if (normalized.includes('商业')) return 'centralCommercial';
  if (normalized.includes('会所')) return 'clubhouse';
  if (normalized.includes('物业')) return 'propertyManagement';
  if (normalized.includes('社区')) return 'communityService';
  if (normalized.includes('幼儿园')) return 'kindergarten';
  if (normalized.includes('产权车位')) return 'undergroundPropertyParking';
  if (normalized.includes('使用权车位')) return 'undergroundUseRightParking';
  if (normalized.includes('人防车位')) return 'civilDefenseParking';
  if (normalized.includes('地上车位')) return 'aboveGroundParking';
  if (normalized.includes('车库') || normalized.includes('地下车位')) return 'undergroundGarage';
  if (normalized.includes('设备')) return 'equipmentRoom';
  if (normalized.includes('人防')) return 'civilDefenseWorks';
  const categoryText = String(category || '');
  if (categoryText.includes('住宅')) return 'highRiseResidential';
  if (categoryText.includes('商业')) return 'streetRetail';
  return null;
}
