...
function prefixedGroups(scope: string, prefix: string): GroupInput[] {
  return residentialCommon.map((item) => ({
    ...
    group: item.group,
    code: item.code,
    details: item.details.map((name) => name.startsWith(prefix) || ['内墙抹灰', '天棚腻子/粗装', '楼地面找平', '公区粗装', '楼梯间粗装', '屋面工程', '屋面防水', '屋面保温', '卫生间防水', '厨房防水', '阳台防水', '外墙保温', '厨房烟道', '卫生间排气道', '成品烟道安装', '屋面风帽', '防火止回阀', '烟道洞口封堵修补', '铝合金窗', '铝合金门', '空调百叶', '设备百叶', '百叶栏杆/格栅栏杆', '阳台栏杆', '阳台栏板', '护窗栏杆', '楼梯栏杆/扶手', '连廊栏杆', '住宅入户门', '单元门/大堂门', '管井门', '楼梯间防火门', '前室防火门', '设备房防火门', '主楼地下室底板防水', '主楼地下室外墙防水', '主楼地下室顶板防水', '后浇带防水加强', '施工缝/变形缝防水', '集水坑/电梯基坑防水', '穿墙套管防水封堵', '顶板保护层', '顶板排水板', '顶板滤水层/土工布', '顶板覆土', '消防车道顶板加强', '地坪基层', '地库找平层', '耐磨地坪', '环氧/固化地坪', '设备房地坪', '地下室砌体', '地下室内墙抹灰', '地下室天棚腻子/涂料', '地下室墙面防霉涂料', '设备房粗装', '楼梯间粗装', '排水沟', '集水坑土建', '排水沟盖板', '电梯基坑排水处理', '地下室防火门', '管井防火门', '防火窗'].includes(name) ? name : `${prefix}${name}`),
