...
function getOrCreateGroup(groupMap: Map<string, Group>, groupName: string) {
  ...
  const created: Group = { id, name: groupName, amount: 0, rows: 0, filled: 0, entries: [] };
  groupMap.set(groupName, created);
  return created;
}
...
export async function ProfessionalDetailPage(props: DetailPageProps) {
  ...
  for (const dict of rawLeafRows) {
    ...
    if (hasAny(dict.applicableProductType, ['全项目', '项目整体', '项目共用'])) {
      ...
      for (const product of activeProducts) {
        ...
        const groupName = setting.standalone ? product.name : setting.groupName;
        targetGroupNames.add(groupName);
        if (!setting.standalone) redirectedProductRows += 1;
      }
    }
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1680 }}>
    <div className="page-header"><div><p className="eyebrow">{props.eyebrow}</p><h1 className="title">{project.name}</h1><p className="subtitle">{props.subtitle} 当前按“启用业态 + 成本归属规则”分组展示；未启用业态不生成分组。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/summary`} className="btn btn-primary">目标成本汇总</Link><Link href={`/projects/${project.id}/cost-mapping`} className="btn">导入科目映射</Link><Link href={`/projects/${project.id}/product-maintenance`} className="btn">业态维护</Link><Link href={`/projects/${project.id}/overview`} className="btn">项目概况</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
    {props.saved === '1' ? <div className="card" style={{ marginBottom: 16, borderColor: '#b2f2bb' }}>{props.title}已批量保存。</div> : null}
    {hiddenDictionaryRows || hiddenCostRows || redirectedProductRows ? <div className="card" style={{ marginBottom: 16, borderColor: '#ffd8a8', background: '#fff9db' }}>已隐藏未启用业态相关科目 {hiddenDictionaryRows} 行、成本行 {hiddenCostRows} 行；有 {redirectedProductRows} 个业态明细按成本归属规则归入地下室、公共配套、道路景观或设备安装等分组。</div> : null}
