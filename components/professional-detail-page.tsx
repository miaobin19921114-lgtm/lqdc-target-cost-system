...
type DetailEntry = { entryId: string; dict: any; saved: any; amount: number; groupName: string };
...
function getOrCreateSecond(map: Map<string, SubjectSecondGroup>, name: string) {
  ...
  if (found) return found;
  const created: SubjectSecondGroup = { id: `subject-second-${key}`, name: key, amount: 0, rows: 0, filled: 0, children: new Map() };
  map.set(key, created);
  return created;
}
...
function getOrCreateThird(second: SubjectSecondGroup, name: string) {
  ...
  if (found) return found;
  const created: SubjectThirdGroup = { id: `${second.id}-third-${key}`, name: key, amount: 0, rows: 0, filled: 0, entries: [] };
  second.children.set(key, created);
  return created;
}

function buildSubjectTree(entries: DetailEntry[]) {
  ...
  return Array.from(secondMap.values()).map((second) => ({ ...second, childRows: Array.from(second.children.values()) }));
}
...
export async function ProfessionalDetailPage(props: DetailPageProps) {
  ...
  function renderEntryTable(entries: DetailEntry[]) {
    return <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 1700, borderCollapse: 'collapse', fontSize: 12 }}><thead><tr style={{ background: '#fff' }}>{['编码', '末级科目', '专业/部位', '测算依据', '工程量', '单位', '含税单价', '税率', '含税金额', '不含税金额', '税额', '分摊方式', '备注', '状态'].map((head, index) => <th key={head} style={{ ...(index === 0 ? stickyCode : index === 1 ? stickyName : cell), textAlign: 'left', color: '#475467' }}>{head}</th>)}</tr></thead><tbody>{entries.map((entry, index) => { const dict = entry.dict; const saved = entry.saved; const amount = entry.amount; const suggestion = suggestQuantityFromOverview(project, dict); const quantity = saved ? Number(saved.quantity || 0) : suggestion.quantity; const unit = saved?.unit || suggestion.unit || dict.unit || ''; const unitPrice = Number(saved?.taxInclusiveUnitPrice || 0); const isFilled = amount > 0; const taxRateText = saved ? `${Number(saved.taxRate || 0) * 100}%` : dict.defaultTaxRate || '9%'; return <tr key={entry.entryId} style={{ background: isFilled ? '#f8fff9' : index % 2 ? '#fff' : '#fcfdff' }}><td style={stickyCode}>{dict.costCode || '-'}</td><td style={stickyName}>{dict.detailSubject || '-'}</td><td style={cell}>{dict.thirdSubject || dict.secondSubject || '-'}</td><td style={cell}><input form={formId} type="hidden" name="dictionaryRowId" value={entry.entryId} />{saved ? <input form={formId} type="hidden" name={entryKey(entry.entryId, 'costLineId')} value={saved.id} /> : null}<input form={formId} type="hidden" name={entryKey(entry.entryId, 'regionOrProductType')} value={entry.groupName} /><input form={formId} name={entryKey(entry.entryId, 'measureBasis')} defaultValue={saved?.measureBasis || dict.measureBasis || ''} style={{ ...inputStyle, minWidth: 180 }} />{!saved && suggestion.source ? <div className="meta">默认取数：{suggestion.source}</div> : null}</td><td style={{ ...cell, padding: 0 }}><input form={formId} name={entryKey(entry.entryId, 'quantity')} type="number" step="0.01" defaultValue={quantity || ''} placeholder="工程量" style={inputStyle} /></td><td style={{ ...cell, padding: 0 }}><input form={formId} name={entryKey(entry.entryId, 'unit')} defaultValue={unit} style={{ ...inputStyle, minWidth: 70 }} /></td><td style={{ ...cell, padding: 0 }}><input form={formId} name={entryKey(entry.entryId, 'taxInclusiveUnitPrice')} type="number" step="0.01" defaultValue={unitPrice || ''} placeholder="单价" style={inputStyle} /></td><td style={{ ...cell, padding: 0 }}><input form={formId} name={entryKey(entry.entryId, 'taxRate')} defaultValue={taxRateText} style={{ ...inputStyle, minWidth: 68 }} /></td><td style={{ ...cell, textAlign: 'right', fontWeight: 900 }}>{fmt(amount)}</td><td style={{ ...cell, textAlign: 'right' }}>{fmt(saved?.taxExclusiveAmount || 0)}</td><td style={{ ...cell, textAlign: 'right' }}>{fmt(saved?.taxAmount || 0)}</td><td style={{ ...cell, padding: 0 }}><input form={formId} name={entryKey(entry.entryId, 'allocationMethod')} defaultValue={saved?.allocationMethod || dict.targetAllocationMethod || '建筑面积分摊'} style={{ ...inputStyle, minWidth: 140 }} /></td><td style={{ ...cell, padding: 0 }}><input form={formId} name={entryKey(entry.entryId, 'remark')} defaultValue={saved?.remark || dict.remark || ''} placeholder="备注" style={{ ...inputStyle, minWidth: 130 }} /></td><td style={{ ...cell, color: isFilled ? '#2f9e44' : '#98a2b3', fontWeight: 800 }}>{isFilled ? '已填' : (suggestion.quantity ? '已带入' : '未填')}</td></tr>; })}</tbody></table></div>;
  }

  return <main className="page"><div className="container" style={{ maxWidth: 1680 }} data-detail-scope={scopeId}>
    <div className="page-header"><div><p className="eyebrow">{props.eyebrow}</p><h1 className="title">{project.name}</h1><p className="subtitle">{props.subtitle} 当前按“成本归属 + 二级科目 + 三级科目 + 末级明细”分组展示；未启用业态、虚拟归属组和跨专业归属组不生成分组。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/summary`} className="btn btn-primary">目标成本汇总</Link><Link href={`/projects/${project.id}/cost-mapping`} className="btn">导入科目映射</Link><Link href={`/projects/${project.id}/product-maintenance`} className="btn">业态维护</Link><Link href={`/projects/${project.id}/overview`} className="btn">项目概况</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
    ...
    <section className="card" style={{ padding: 0, overflow: 'hidden' }}><div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)', background: '#f8fafc', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}><div><b>{props.title}｜科目树填报</b><div className="meta">先按成本归属分组，再按二级/三级科目折叠；前期费可看到规费、设计、勘察、三通一平、临设、保险担保等层级。</div></div><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}><ProfessionalDetailFoldControls scopeId={scopeId} /><button form={formId} className="btn btn-primary" style={{ minHeight: 34 }}>整表批量保存</button></div></div><form id={formId} action={`/api/projects/${project.id}/professional-costs/batch`} method="post" /><input form={formId} type="hidden" name="professionalGroup" value={props.professionalGroup} /><input form={formId} type="hidden" name="returnPath" value={props.returnPath} />
      <div style={{ maxHeight: '72vh', overflow: 'auto', padding: 12 }}>
        {visibleGroups.length === 0 ? <p className="meta">{props.emptyText} 请先在项目概况/业态维护中启用对应业态。</p> : visibleGroups.map((group) => <details key={group.id} data-cost-detail-group open style={{ border: '1px solid var(--border)', borderRadius: 10, marginBottom: 10, overflow: 'hidden', background: '#fff' }}><summary style={{ cursor: 'pointer', padding: 12, background: '#e9f7f8', display: 'grid', gridTemplateColumns: '1fr 130px 150px 110px 120px', gap: 10, alignItems: 'center', fontWeight: 900 }}><span>成本归属｜{group.name}</span><span>已填 {group.filled}/{group.rows}</span><span style={{ textAlign: 'right' }}>{fmt(group.amount)}</span><span style={{ textAlign: 'right' }}><GroupSaveButton formId={formId} groupId={group.id} /></span><span style={{ textAlign: 'right' }}>展开/收起</span></summary><div style={{ padding: 10 }}>{buildSubjectTree(group.entries).map((second) => <details key={`${group.id}-${second.id}`} data-cost-detail-group open style={{ border: '1px solid #eef2f6', borderRadius: 8, marginBottom: 8, overflow: 'hidden', background: '#fff' }}><summary style={{ cursor: 'pointer', padding: 10, background: '#f8fafc', display: 'grid', gridTemplateColumns: '1fr 130px 150px 120px', gap: 10, alignItems: 'center', fontWeight: 800 }}><span>二级｜{second.name}</span><span>已填 {second.filled}/{second.rows}</span><span style={{ textAlign: 'right' }}>{fmt(second.amount)}</span><span style={{ textAlign: 'right' }}>展开/收起</span></summary><div style={{ padding: 8 }}>{second.childRows.map((third) => <details key={`${group.id}-${third.id}`} data-cost-detail-group open style={{ border: '1px solid #eef2f6', borderRadius: 8, marginBottom: 8, overflow: 'hidden', background: '#fff' }}><summary style={{ cursor: 'pointer', padding: 10, background: '#fcfdff', display: 'grid', gridTemplateColumns: '1fr 130px 150px 120px', gap: 10, alignItems: 'center' }}><b>三级｜{third.name}</b><span>已填 {third.filled}/{third.rows}</span><span style={{ textAlign: 'right', fontWeight: 800 }}>{fmt(third.amount)}</span><span style={{ textAlign: 'right' }}>展开/收起</span></summary>{renderEntryTable(third.entries)}</details>)}</div></details>)}</div></details>)}
      </div></section>
  </div></main>;
}
