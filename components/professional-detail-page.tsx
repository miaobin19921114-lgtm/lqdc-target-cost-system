...
export async function ProfessionalDetailPage(props: DetailPageProps) {
  ...
  const dictionaryRows = await prisma.costDictionaryRow.findMany({
    where: { projectId: props.projectId, enabled: { not: '否' }, OR: ors },
      ...
      <div className="container" style={{ maxWidth: 1600 }}>
        ...
        <section className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)', background: '#f8fafc', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
            <div><b>{props.title}｜批量填报</b><div className="meta">{props.note} 左侧科目冻结，自动插入小计行，填完多行后统一保存。</div></div>
              ...
              <tbody>
                {dictionaryRows.length === 0 ? <tr><td colSpan={17} style={{ padding: 18, color: '#667085', textAlign: 'center' }}>{props.emptyText}</td></tr> : dictionaryRows.map((dict, index) => {
                  ...
                  return <Fragment key={dict.id}>
                    {showGroup ? <tr style={{ background: '#e9f7f8', fontWeight: 900 }}><td colSpan={5} style={{ ...cell, ...frozen(0, '#e9f7f8'), color: '#0f4c5c' }}>{groupName} 小计</td><td colSpan={6} style={cell}>已填 {group?.filled || 0} / {group?.rows || 0} 行</td><td style={{ ...cell, textAlign: 'right' }}>{fmt(group?.amount || 0)}</td><td colSpan={5} style={cell}></td></tr> : null}
                    <tr style={{ background: isFilled ? '#f8fff9' : index % 2 ? '#fff' : '#fcfdff' }}>
                      ...
                      <td style={{ ...cell, ...frozen(560), minWidth: 240, fontWeight: 800 }}>{dict.detailSubject || dict.thirdSubject || dict.secondSubject || '-'}</td>
