  ...
  return <main className="page" style={{ padding: 0 }}>
    <div data-detail-scope={scopeId} style={{ maxWidth: 1840, margin: '0 auto', padding: 12 }}>
      ...
      <div className="container" style={{ maxWidth: 'none', width: '100%', padding: 0 }}>
        <div className="page-header"><div><p className="eyebrow">{props.eyebrow}</p><h1 className="title">{project.name}</h1><p className="subtitle">{props.subtitle} 底层按“测算指标 × 含量/系数 = 工程量/计费基数”计算；工程量单位优先按末级科目单位，勾选手动后才按手填工程量保存。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/summary`} className="btn btn-primary">目标成本汇总</Link><Link href={`/projects/${project.id}/product-maintenance`} className="btn">业态维护</Link><Link href={`/projects/${project.id}/overview`} className="btn">项目概况</Link><Link href={`/projects/${project.id}`} className="btn">返回项目测算中心</Link></div></div>
        <VersionContextBar projectName={project.name} versionName={version?.name} versionStatus={version?.status} extra={[['启用业态', activeProducts.length], ['可填明细', visibleRows]]} />
        {props.saved === '1' ? <StatusNotice title="成本明细已保存" tone="success">成本明细已保存。请生成明细测算结果，汇总至目标成本测算表。</StatusNotice> : null}
        {locked ? <StatusNotice title="当前版本已锁定" tone="danger">当前版本已锁定，仅支持查看。如需调整数据，请在版本管理中复制为新版本后编辑。</StatusNotice> : null}
        {hiddenDictionaryRows || hiddenCostRows || redirectedProductRows ? <StatusNotice title="已按当前配置过滤明细" tone="warning">已隐藏未启用、配置未触发或跨专业科目 {hiddenDictionaryRows} 行，隐藏成本行 {hiddenCostRows} 行；有 {redirectedProductRows} 个业态明细按成本归属规则重定向。</StatusNotice> : null}
        <div className="summary-strip"><div className="stat"><div className="stat-label">本专业科目数</div><div className="stat-value">{visibleRows}</div></div><div className="stat"><div className="stat-label">已填报行数</div><div className="stat-value">{filledRows}</div></div><div className="stat"><div className="stat-label">待填报行数</div><div className="stat-value">{Math.max(visibleRows - filledRows, 0)}</div></div><div className="stat"><div className="stat-label">含税金额合计</div><div className="stat-value">{fmt(totalInclusive)}</div></div><div className="stat"><div className="stat-label">异常行数</div><div className="stat-value">{hiddenDictionaryRows + hiddenCostRows}</div></div></div>
        <section className="card" style={{ padding: 0, overflow: 'hidden' }}><div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)', background: '#f8fafc', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}><div><b>{props.title}｜业态归属 + 科目树填报</b><div className="meta">顶层排序：项目整体共用、住宅、商业、地下室/车位、配套、其他；启用业态优先按地产产品逻辑顺序显示，二级、三级科目均可单独保存。</div></div><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}><ProfessionalDetailFoldControls scopeId={scopeId} /><button form={formId} className="btn btn-primary" style={{ minHeight: 34 }} disabled={locked} title={locked ? '当前版本已锁定，仅支持查看。' : undefined}>整表批量保存</button></div></div><form id={formId} action={`/api/projects/${project.id}/professional-costs/batch`} method="post" /><input form={formId} type="hidden" name="professionalGroup" value={props.professionalGroup} /><input form={formId} type="hidden" name="returnPath" value={props.returnPath} />
          <div style={{ maxHeight: '72vh', overflow: 'auto', padding: 12 }}>
            {visibleGroups.length === 0 ? <EmptyState title={props.emptyText}>请先在项目概况或业态维护中启用对应业态，并检查概况表专项配置开关是否已打开。</EmptyState> : visibleGroups.map((group) => <details key={group.id} data-cost-detail-group open style={{ border: '1px solid var(--border)', borderRadius: 10, marginBottom: 10, overflow: 'hidden', background: '#fff' }}><summary style={{ cursor: 'pointer', padding: 12, background: '#e9f7f8', display: 'grid', gridTemplateColumns: '1fr 130px 150px 120px 120px', gap: 10, alignItems: 'center', fontWeight: 900 }}><span>成本归属｜{group.name}</span><span>已填 {group.filled}/{group.rows}</span><span style={{ textAlign: 'right' }}>{fmt(group.amount)}</span><span style={{ textAlign: 'right' }}><GroupSaveButton formId={formId} groupId={group.id} label="保存归属组" disabled={locked} /></span><span style={{ textAlign: 'right' }}>展开/收起</span></summary><div style={{ padding: 10 }}>{buildSubjectTree(group.entries).map((second) => <details key={`${group.id}-${second.id}`} data-cost-detail-group open style={{ border: '1px solid #eef2f6', borderRadius: 8, marginBottom: 8, overflow: 'hidden', background: '#fff' }}><summary style={{ cursor: 'pointer', padding: 10, background: '#f8fafc', display: 'grid', gridTemplateColumns: '1fr 130px 150px 120px 120px', gap: 10, alignItems: 'center', fontWeight: 800 }}><span>二级｜{second.name}</span><span>已填 {second.filled}/{second.rows}</span><span style={{ textAlign: 'right' }}>{fmt(second.amount)}</span><span style={{ textAlign: 'right' }}><GroupSaveButton formId={formId} groupId={`${group.id}__${second.id}`} label="保存二级" disabled={locked} /></span><span style={{ textAlign: 'right' }}>展开/收起</span></summary><div style={{ padding: 8 }}>{second.childRows.map((third) => <details key={`${group.id}-${third.id}`} data-cost-detail-group open style={{ border: '1px solid #eef2f6', borderRadius: 8, marginBottom: 8, overflow: 'hidden', background: '#fff' }}><summary style={{ cursor: 'pointer', padding: 10, background: '#fcfdff', display: 'grid', gridTemplateColumns: '1fr 130px 150px 120px 120px', gap: 10, alignItems: 'center' }}><b>三级｜{third.name}</b><span>已填 {third.filled}/{third.rows}</span><span style={{ textAlign: 'right' }}>{fmt(third.amount)}</span><span style={{ textAlign: 'right' }}><GroupSaveButton formId={formId} groupId={`${group.id}__${second.id}__${third.id}`} label="保存三级" disabled={locked} /></span><span style={{ textAlign: 'right' }}>展开/收起</span></summary>{renderEntryTable(third.entries, [`${group.id}__${second.id}`, `${group.id}__${second.id}__${third.id}`])}</details>)}</div></details>)}</div></details>)}
          </div>
        </section>
      </div>
    </div>
