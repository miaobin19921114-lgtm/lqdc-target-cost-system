    ...
    {templates.length === 0 ? <section className="card"><h2>暂无模板</h2><p className="meta">部署迁移完成后会自动生成“住宅开发目标成本标准模板”。</p></section> : <div style={{ display: 'grid', gap: 16 }}>{templates.map((tpl) => {
      ...
      return <section key={tpl.id} className="card"><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'start' }}><div><span className="badge">{canEdit ? '我的模板' : '系统只读模板'}</span><h2 style={{ marginTop: 10 }}>{tpl.name}</h2><p className="meta">{tpl.description || '暂无说明'}</p></div><div style={{ display: 'grid', gap: 8, justifyItems: 'end' }}><div className="stat"><div className="stat-label">启用/停用/规则/税率</div><div className="stat-value">{activeProducts.length}/{disabledProducts.length}/{tpl.costRules.length}/{tpl.taxRules.length}</div></div>{!canEdit && userId ? <form action="/api/templates/products" method="post"><input type="hidden" name="action" value="copy" /><input type="hidden" name="templateId" value={tpl.id} /><button className="btn btn-primary">复制为我的模板后编辑</button></form> : null}</div></div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 12, marginTop: 12 }}>
          ...
          <div style={{ border: '1px solid #eef2f6', borderRadius: 10, padding: 12 }}><b>科目/税率概览</b><p className="meta">科目规则 {tpl.costRules.length} 条；税率规则 {tpl.taxRules.length} 条。系统默认科目和税率也只读，后续编辑会进入个人模板。</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 12, marginTop: 10 }}><div>{tpl.costRules.slice(0, 8).map((r) => <div key={r.id} style={{ padding: '6px 0', borderBottom: '1px solid #f1f3f5' }}><b>{r.subjectName}</b><div className="meta">{r.sourceTable || '-'} · {r.measureBasis || '-'}</div></div>)}</div><div>{tpl.taxRules.map((r) => <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #f1f3f5' }}><span>{r.name}</span><b>{Number(r.rate) * 100}%</b></div>)}</div></div></div>
        </div></section>;
    })}</div>}
  </div></main>;
}
