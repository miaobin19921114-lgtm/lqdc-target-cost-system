...
function allocationSelect(name: string, value?: string | null) {
  return <select name={name} defaultValue={value || ''} style={{ width: '100%', height: 30, border: '1px solid #d9e2ec', borderRadius: 6 }}>
    {allocationOptions.map((option) => <option key={option || 'inherit'} value={option}>{option || '继承上级/系统默认'}</option>)}
  </select>;
}

function ruleText(value?: string | null) {
...
export default async function TemplateRulesPage({ params, searchParams }: { params: { id: string }, searchParams?: Record<string, string | undefined> }) {
  ...
  const allocationRuleCount = sortedCostRules.filter((item) => item.allocationMethod || parseTemplateAllocationRules(item.allocationMethod, item.remark).landVatAllocationMethod || parseTemplateAllocationRules(item.allocationMethod, item.remark).incomeTaxAllocationMethod).length;
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1500 }}>
    ...
    <section className="card" style={{ marginBottom: 16, borderColor: '#c5eef3', background: '#f8fbff' }}><h2>分摊规则模板</h2><p className="meta">一条科目规则现在对应三套口径：经营分摊、土增税清算、所得税成本对象。空值代表继承上级或系统默认，项目特殊情况以后只做项目覆盖。</p><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 1460, borderCollapse: 'collapse', fontSize: 12 }}><thead><tr>{['编码', '科目', '精度建议', '经营分摊规则', '土增税清算规则', '所得税成本对象规则', '操作'].map((head) => <th key={head} style={{ padding: 8, borderBottom: '1px solid var(--border)', textAlign: 'left', color: 'var(--muted)' }}>{head}</th>)}</tr></thead><tbody>{sortedCostRules.map((rule) => {
      const parsed = parseTemplateAllocationRules(rule.allocationMethod, rule.remark);
      return <tr key={`allocation-${rule.id}`}><td style={{ padding: 8, borderBottom: '1px solid var(--border)', fontWeight: 800 }}>{rule.costCode || '-'}</td><td style={{ padding: 8, borderBottom: '1px solid var(--border)', fontWeight: 800 }}>{rule.subjectName}</td><td style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{precisionAdvice(rule)}</td><td style={{ padding: 6, borderBottom: '1px solid var(--border)', minWidth: 190 }}>{canEdit ? <form id={`alloc-${rule.id}`} action="/api/templates/cost-rules" method="post"><input type="hidden" name="action" value="update" /><input type="hidden" name="ruleId" value={rule.id} /><input type="hidden" name="costCode" value={rule.costCode || ''} /><input type="hidden" name="category" value={rule.category || ''} /><input type="hidden" name="subjectName" value={rule.subjectName} /><input type="hidden" name="sourceTable" value={rule.sourceTable || ''} /><input type="hidden" name="measureBasis" value={rule.measureBasis || ''} /><input type="hidden" name="unit" value={rule.unit || ''} /><input type="hidden" name="defaultTaxRate" value={String(Number(rule.defaultTaxRate || 0.09))} /><input type="hidden" name="sortOrder" value={String(rule.sortOrder)} /><input type="hidden" name="remark" value={rule.remark || ''} />{allocationSelect('operatingAllocationMethod', parsed.operatingAllocationMethod)}</form> : ruleText(parsed.operatingAllocationMethod)}</td><td style={{ padding: 6, borderBottom: '1px solid var(--border)', minWidth: 190 }}>{canEdit ? allocationSelect('landVatAllocationMethod', parsed.landVatAllocationMethod) : ruleText(parsed.landVatAllocationMethod)}</td><td style={{ padding: 6, borderBottom: '1px solid var(--border)', minWidth: 190 }}>{canEdit ? allocationSelect('incomeTaxAllocationMethod', parsed.incomeTaxAllocationMethod) : ruleText(parsed.incomeTaxAllocationMethod)}</td><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}>{canEdit ? <button form={`alloc-${rule.id}`} className="btn" style={{ padding: '5px 8px' }}>保存</button> : '只读'}</td></tr>;
    })}</tbody></table></div></section>

    <section className="card" style={{ marginBottom: 16 }}><h2>成本科目规则</h2><p className="meta">维护科目编码、测算依据、单位、默认税率和通用经营分摊方式。后续新建项目会复制你的个人模板规则。</p>{canEdit ? <form action="/api/templates/cost-rules" method="post" style={{ display: 'grid', gridTemplateColumns: '110px 120px 1fr 180px 1fr 90px 100px 180px 80px', gap: 8, alignItems: 'end', marginBottom: 12 }}><input type="hidden" name="action" value="create" /><input type="hidden" name="templateId" value={template.id} /><label>编码<input name="costCode" style={{ width: '100%', height: 34 }} /></label><label>分类<input name="category" style={{ width: '100%', height: 34 }} /></label><label>科目名称<input name="subjectName" required style={{ width: '100%', height: 34 }} /></label><label>来源表<input name="sourceTable" style={{ width: '100%', height: 34 }} /></label><label>测算依据<input name="measureBasis" style={{ width: '100%', height: 34 }} /></label><label>单位<input name="unit" style={{ width: '100%', height: 34 }} /></label><label>税率<input name="defaultTaxRate" defaultValue="9%" style={{ width: '100%', height: 34 }} /></label><label>经营分摊<input name="operatingAllocationMethod" style={{ width: '100%', height: 34 }} /></label><button className="btn btn-primary">新增</button></form> : null}<div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 1380, borderCollapse: 'collapse', fontSize: 12 }}><thead><tr>{['编码', '分类', '科目名称', '来源表', '测算依据', '单位', '税率%', '经营分摊', '排序', '操作'].map((head) => <th key={head} style={{ padding: 8, borderBottom: '1px solid var(--border)', textAlign: 'left', color: 'var(--muted)' }}>{head}</th>)}</tr></thead><tbody>{sortedCostRules.map((rule) => {
      const parsed = parseTemplateAllocationRules(rule.allocationMethod, rule.remark);
      return canEdit ? <tr key={rule.id}><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}><input form={`cr-${rule.id}`} name="costCode" defaultValue={rule.costCode || ''} style={{ width: 95, height: 30 }} /></td><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}><input form={`cr-${rule.id}`} name="category" defaultValue={rule.category || ''} style={{ width: 105, height: 30 }} /></td><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}><input form={`cr-${rule.id}`} name="subjectName" defaultValue={rule.subjectName} style={{ width: '100%', height: 30 }} /></td><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}><input form={`cr-${rule.id}`} name="sourceTable" defaultValue={rule.sourceTable || ''} style={{ width: 170, height: 30 }} /></td><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}><input form={`cr-${rule.id}`} name="measureBasis" defaultValue={rule.measureBasis || ''} style={{ width: '100%', height: 30 }} /></td><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}><input form={`cr-${rule.id}`} name="unit" defaultValue={rule.unit || ''} style={{ width: 70, height: 30 }} /></td><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}><input form={`cr-${rule.id}`} name="defaultTaxRate" defaultValue={fmt(percent(rule.defaultTaxRate))} style={{ width: 80, height: 30 }} /></td><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}><input form={`cr-${rule.id}`} name="operatingAllocationMethod" defaultValue={parsed.operatingAllocationMethod || ''} style={{ width: 160, height: 30 }} /></td><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}><input form={`cr-${rule.id}`} name="sortOrder" type="number" defaultValue={rule.sortOrder} style={{ width: 70, height: 30 }} /></td><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}><form id={`cr-${rule.id}`} action="/api/templates/cost-rules" method="post"><input type="hidden" name="action" value="update" /><input type="hidden" name="ruleId" value={rule.id} /><input type="hidden" name="landVatAllocationMethod" value={parsed.landVatAllocationMethod || ''} /><input type="hidden" name="incomeTaxAllocationMethod" value={parsed.incomeTaxAllocationMethod || ''} /><button className="btn" style={{ padding: '5px 8px' }}>保存</button><button className="btn" name="action" value="delete" style={{ padding: '5px 8px', color: '#c92a2a', marginLeft: 6 }}>删除</button></form></td></tr> : <tr key={rule.id}><td style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{rule.costCode || '-'}</td><td style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{rule.category || '-'}</td><td style={{ padding: 8, borderBottom: '1px solid var(--border)', fontWeight: 800 }}>{rule.subjectName}</td><td style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{rule.sourceTable || '-'}</td><td style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{rule.measureBasis || '-'}</td><td style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{rule.unit || '-'}</td><td style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{fmt(percent(rule.defaultTaxRate))}</td><td style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{ruleText(parsed.operatingAllocationMethod)}</td><td style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{rule.sortOrder}</td><td style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>只读</td></tr>;
    })}</tbody></table></div></section>
