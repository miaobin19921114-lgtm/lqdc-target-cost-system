  ...
  return (
    <main style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1.05fr .95fr', background: 'linear-gradient(135deg,#0f3a4a 0%,#1f6f78 48%,#eef3f8 48%,#eef3f8 100%)' }}>
      <section style={{ color: '#fff', padding: '72px 60px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <h1 style={{ fontSize: 42, lineHeight: 1.12, margin: '24px 0 12px' }}>地产目标成本<br />测算平台</h1>
      ...
      <section style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
        <div style={{ width: '100%', maxWidth: 430, background: '#fff', border: '1px solid #d9e2ec', borderRadius: 22, boxShadow: '0 24px 70px rgba(15,58,74,.18)', padding: 30 }}>
          <p style={{ margin: 0, color: '#0f4c5c', fontWeight: 900, fontSize: 13 }}>地产目标成本测算平台</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <span style={{ height: 42, borderRadius: 10, border: '1px solid #d9e2ec', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#98a2b3', background: '#f8fafc', fontWeight: 800 }}>微信登录（即将开放）</span>
            <button disabled style={{ height: 42, borderRadius: 10, border: '1px solid #d9e2ec', background: '#f8fafc', color: '#98a2b3', fontWeight: 800 }}>短信验证码（即将开放）</button>
          </div>
