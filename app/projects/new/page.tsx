...
export default async function NewProjectPage() {
  ...
  const productGroups = template ? Array.from(new Set(template.products.map((item) => item.category))).map((category) => ({
    category,
    items: template.products.filter((item) => item.category === category)
  })) : [];
  ...
  return <main className="page"><form action="/api/projects" method="post" className="container" style={{ maxWidth: 1100 }}>
    ...
    <section className="card"><span className="badge">第3步</span><h2>确认本项目业态</h2><p className="meta">按树状结构选择：先看大类，再勾选具体业态。未勾选的不生成到项目概况表。</p>{productGroups.length ? <div style={{ display: 'grid', gap: 12 }}>{productGroups.map((group) => <div key={group.category} style={{ border: '1px solid #d9e2ec', borderRadius: 12, overflow: 'hidden' }}><div style={{ background: '#f0fbfc', padding: '10px 12px', fontWeight: 900, color: '#0f4c5c' }}>{group.category}</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10, padding: 12 }}>{group.items.map((item) => <label key={item.id} style={{ display: 'flex', gap: 8, border: '1px solid #eef2f6', borderRadius: 10, padding: 10, background: '#fff' }}><input name="templateProductIds" type="checkbox" value={item.id} defaultChecked={item.isSaleable || item.participateAllocation} /><span><b>{item.name}</b><div className="meta">{item.isSaleable ? '可售' : '不可售'} · {item.participateAllocation ? '参与分摊' : '不参与分摊'}</div></span></label>)}</div></div>)}</div> : null}</section>
