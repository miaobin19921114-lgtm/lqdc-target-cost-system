...
export default async function ProjectOperatingReport({ params }: { params: { id: string } }) {
  ...
  return <main className="page report-page"><div className="container" style={{ maxWidth: 1180 }}>
    ...
    <section className="card" style={{ marginTop: 16 }}><h2>一、经营结论</h2><ol style={{ margin: 0, paddingLeft: 20, lineHeight: 1.8 }}><li>项目整体税后净利 {fmt(tax.netProfit)} 元，销售净利率 {pct(netMargin)}。</li><li>含税收入 {fmt(revenue.taxInclusive)} 元，含税目标成本 {fmt(cost.taxInclusive)} 元。</li><li>毛利率 {pct(grossMargin)}，税费合计 {fmt(tax.totalTax)} 元。</li><li>已按有效末级成本行汇总，排除非末级历史成本 {effective.ignoredNonLeaf} 行。</li></ol></section>
    ...
    <section className="card" style={{ marginTop: 16 }}><h2>三、数据口径提示</h2><p className="meta">本报告按当前启用版本、启用业态、末级成本及 Excel 导入四级科目统计；土增税与所得税为测算口径，后续可结合清算规则继续深化。</p>{effective.importedLeafRows ? <div style={{ marginTop: 12, border: '1px solid #b2f2bb', background: '#f0fff4', borderRadius: 10, padding: 10 }}>已计入 {effective.importedLeafRows} 条 Excel 导入/临时四级成本科目。</div> : null}</section>
