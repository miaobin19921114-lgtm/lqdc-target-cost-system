...
export default async function ProjectOperatingReport({ params }: { params: { id: string } }) {
  ...
  return <main className="page report-page"><div className="container" style={{ maxWidth: 1180 }}>
    ...
    <section className="card" style={{ marginTop: 16 }}><h2>四、矩阵自动结论</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 }}><div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 12 }}><div className="meta">最差组合</div><div style={{ fontWeight: 900, color: statusColor(worstCell.result.netProfit) }}>售价{pct(worstCell.price - 1)} / 成本{pct(worstCell.costFactor - 1)}</div><div className="meta">税后净利 {fmt(worstCell.result.netProfit)}，净利率 {pct(worstCell.result.netMargin)}</div></div><div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 12 }}><div className="meta">最好组合</div><div style={{ fontWeight: 900, color: statusColor(bestCell.result.netProfit) }}>售价{pct(bestCell.price - 1)} / 成本{pct(bestCell.costFactor - 1)}</div><div className="meta">税后净利 {fmt(bestCell.result.netProfit)}，净利率 {pct(bestCell.result.netMargin)}</div></div><div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 12 }}><div className="meta">亏损格子</div><div style={{ fontWeight: 900, color: lossCells ? '#e03131' : '#2f9e44' }}>{lossCells} / {matrixCells.length}</div><div className="meta">亏损格子越多，抗压能力越弱</div></div></div></section>
    <section className="card" style={{ marginTop: 16 }}><h2>五、售价 × 成本二维矩阵</h2><p className="meta">单元格显示税后净利 / 净利率；横向为成本变化，纵向为售价变化。</p><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 760, borderCollapse: 'collapse' }}><thead><tr><th style={{ textAlign: 'left', padding: 10, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>售价 \ 成本</th>{costFactors.map((factor) => <th key={factor} style={{ textAlign: 'left', padding: 10, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>成本{pct(factor - 1)}</th>)}</tr></thead><tbody>{sensitivityMatrix.map((row) => <tr key={row.price}><td style={{ padding: 10, borderBottom: '1px solid var(--border)', fontWeight: 900 }}>售价{pct(row.price - 1)}</td>{row.cells.map((cell) => <td key={`${row.price}-${cell.costFactor}`} style={{ padding: 10, borderBottom: '1px solid var(--border)', color: statusColor(cell.result.netProfit), fontWeight: 800 }}>{fmt(cell.result.netProfit)}<div className="meta">{pct(cell.result.netMargin)}</div></td>)}</tr>)}</tbody></table></div></section>
    <section className="card" style={{ marginTop: 16 }}><h2>六、核心经营指标</h2><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 820 }}><tbody>{summaryRows.map(([name, value, unit]) => <tr key={name}><td style={{ padding: 10, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{name}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)', textAlign: 'right', fontWeight: 900, color: String(name).includes('利润') || String(name).includes('净利') ? statusColor(Number(value)) : undefined }}>{unit === 'percent' ? pct(Number(value)) : fmt(value)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{unit === 'percent' ? '' : unit}</td></tr>)}</tbody></table></div></section>
    <section className="card" style={{ marginTop: 16 }}><h2>七、成本结构摘要</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 10 }}>{costRows.map(([name, value]) => <div key={name} style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 12 }}><div className="meta">{name}</div><div style={{ fontWeight: 900, fontSize: 18 }}>{fmt(value)}</div><div className="meta">占不含税成本 {pct(cost.taxExclusive ? Number(value) / cost.taxExclusive : 0)}</div></div>)}</div></section>
    <section className="card" style={{ marginTop: 16 }}><h2>八、税费测算</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10 }}>{taxRows.map(([name, value]) => <div key={name} style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 12 }}><div className="meta">{name}</div><div style={{ fontWeight: 900, fontSize: 18 }}>{fmt(value)}</div></div>)}</div></section>
    <section className="card" style={{ marginTop: 16 }}><h2>九、业态利润摘要</h2><p className="meta">摘要按收入占比快速分摊项目成本和税费；更精细口径以“业态经营利润测算表”为准。</p><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 980, borderCollapse: 'collapse' }}><thead><tr>{['业态', '含税收入', '分摊成本', '分摊税费', '税后净利', '净利率'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 10, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{head}</th>)}</tr></thead><tbody>{productRows.length ? productRows.map((row) => <tr key={row.name}><td style={{ padding: 10, borderBottom: '1px solid var(--border)', fontWeight: 800 }}>{row.name}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.revenueInclusive)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.allocatedCost)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.allocatedTax)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)', fontWeight: 900, color: statusColor(row.netProfit) }}>{fmt(row.netProfit)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{pct(row.revenueInclusive ? row.netProfit / row.revenueInclusive : 0)}</td></tr>) : <tr><td colSpan={6} style={{ padding: 12, color: 'var(--muted)' }}>暂无业态利润数据。</td></tr>}</tbody></table></div></section>
    <section className="card" style={{ marginTop: 16 }}><h2>十、可售业态收入</h2><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 960, borderCollapse: 'collapse' }}><thead><tr>{['业态', '建筑面积', '可售面积', '销售单价', '含税收入', '不含税收入'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 10, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{head}</th>)}</tr></thead><tbody>{productRows.length ? productRows.map((row) => <tr key={row.name}><td style={{ padding: 10, borderBottom: '1px solid var(--border)', fontWeight: 800 }}>{row.name}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.buildingArea)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.saleableArea)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.salePrice)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.revenueInclusive)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.revenueExclusive)}</td></tr>) : <tr><td colSpan={6} style={{ padding: 12, color: 'var(--muted)' }}>暂无可售业态收入。</td></tr>}</tbody></table></div></section>
    <section className="card" style={{ marginTop: 16 }}><h2>十一、数据口径与风险提示</h2><div style={{ display: 'grid', gap: 8 }}>{dataWarnings.length ? dataWarnings.map((item) => <div key={item} style={{ border: '1px solid #ffd8a8', background: '#fff9db', borderRadius: 10, padding: 10 }}>{item}</div>) : <div style={{ border: '1px solid #b2f2bb', background: '#f0fff4', borderRadius: 10, padding: 10 }}>当前核心数据未发现明显缺口。</div>}</div><p className="meta" style={{ marginTop: 12 }}>本报告按当前启用版本、启用业态、末级成本及 Excel 导入四级科目统计；土增税与所得税为测算口径，后续可结合清算规则继续深化。</p></section>
