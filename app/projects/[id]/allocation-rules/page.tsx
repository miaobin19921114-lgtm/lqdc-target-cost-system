...
export default async function AllocationRulesPage({ params, searchParams }: { params: { id: string }, searchParams?: Record<string, string | undefined> }) {
  ...
  const configured = rows.filter((row) => row.targetAllocationMethod || row.landVatAllocationMethod || row.incomeTaxDeductionCategory || row.costAttributionMethod).length;
  const level3Plus = rows.filter((row) => codeLevel(row.costCode, row.subjectLevel) >= 3).length;
  const keyLevel4 = rows.filter((row) => precisionAdvice(row).includes('四级') || precisionAdvice(row).includes('末级')).length;
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1520 }}>
    ...
    <div className="page-header"><div><p className="eyebrow">模板规则</p><h1 className="title">分摊规则配置</h1><p className="subtitle">按 V60/模板词典维护经营、土增税、所得税三套分摊口径。空值代表继承上级或系统默认，项目明细行仍可覆盖。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/cost-allocation`} className="btn btn-primary">查看成本分摊</Link><Link href={`/projects/${project.id}/cost-mapping`} className="btn">科目映射</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
    {statusMessage(searchParams)}
    <div className="summary-strip" style={{ marginBottom: 14 }}><div className="stat"><div className="stat-label">规则科目</div><div className="stat-value">{rows.length}</div></div><div className="stat"><div className="stat-label">已配置</div><div className="stat-value">{configured}</div></div><div className="stat"><div className="stat-label">三级及以下</div><div className="stat-value">{level3Plus}</div></div><div className="stat"><div className="stat-label">关键四级/末级</div><div className="stat-value">{keyLevel4}</div></div></div>
