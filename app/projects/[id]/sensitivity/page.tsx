...
export default async function SensitivityPage({ params }: { params: { id: string } }) {
  ...
  const baseRevenue = revenueFromProjectData({ products: version?.products || [], revenues: version?.revenues || [], commercialRevenueLines, otherRevenueLines, vatRate });
  ...
  return <main className="page"><ProjectTopNav projectId={project.id} projectName={project.name} current="敏感性分析" /><div className="container" style={{ maxWidth: 1180 }}>
    <div className="page-header"><div><p className="eyebrow">敏感性分析</p><h1 className="title">{project.name}</h1><p className="subtitle">基于当前版本的收入、成本和税率，测算售价与成本变化对税后净利的影响。金额单位为万元，单方为元/㎡。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/sensitivity-report`} className="btn btn-primary">敏感性分析报告</Link><Link href={`/projects/${project.id}/profit-analysis`} className="btn">税费利润分析</Link><Link href={`/projects/${project.id}/report-print`} className="btn">打印经营报告</Link></div></div>
