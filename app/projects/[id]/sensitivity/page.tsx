...
export default async function SensitivityPage({ params }: { params: { id: string } }) {
  ...
  const scenarios = [
    ...
    { name: '售价下降 10%', revenueRate: 0.9, costRate: 1 },
    { name: '成本上升 3%', revenueRate: 1, costRate: 1.03 },
    { name: '成本上升 5%', revenueRate: 1, costRate: 1.05 },
    { name: '成本上升 10%', revenueRate: 1, costRate: 1.1 },
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1180 }}>
    <div className="page-header"><div><p className="eyebrow">敏感性测算</p><h1 className="title">{project.name}</h1><p className="subtitle">基于当前版本的收入、成本和税率，自动测算售价/成本变化对税后净利的影响。金额单位为万元，单方为元/㎡。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/sensitivity-report`} className="btn btn-primary">打印版敏感性报告</Link><Link href={`/projects/${project.id}/decision`} className="btn">投决评审</Link><Link href={`/projects/${project.id}/report-print`} className="btn">打印经营报告</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
