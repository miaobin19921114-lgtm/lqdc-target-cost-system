...
export default async function PrintableSensitivityReport({ params }: { params: { id: string } }) {
  ...
  return <main className="print-report">
    <div className="no-print toolbar"><Link href={`/projects/${project.id}/sensitivity`} className="btn">返回敏感性分析</Link><Link href={`/projects/${project.id}/decision`} className="btn">投决评审</Link><Link href={`/projects/${project.id}/report-print`} className="btn">打印经营报告</Link><Link href={`/projects/${project.id}/dashboard-lite`} className="btn">经营总控</Link><span className="btn btn-primary">打印：Ctrl/Cmd + P</span></div>
