...
export default async function PrintableSensitivityReport({ params }: { params: { id: string } }) {
  ...
  function simulate(priceFactor: number, costFactor: number, landFactor = 1) {
    ...
    return { revenueInclusive, costInclusive, totalTax: tax.totalTax, profitBeforeIncomeTax: tax.profitBeforeIncomeTax, incomeTax: tax.incomeTax, netProfit: tax.netProfit, netMargin: revenueInclusive ? tax.netProfit / revenueInclusive : 0 };
  }
  ...
  return <main className="print-report">
    <div className="no-print toolbar"><Link href={`/projects/${project.id}/sensitivity`} className="btn">返回敏感性</Link><Link href={`/projects/${project.id}/report-print`} className="btn">打印版经营报告</Link><Link href={`/projects/${project.id}/dashboard-lite`} className="btn">经营总控</Link><span className="btn btn-primary">打印：Ctrl/Cmd + P</span></div>
    <section className="cover block"><div className="eyebrow">源信达地产目标成本测算系统</div><h1>{project.name}</h1><h2>敏感性测算报告（打印版）</h2><p>当前版本：{version?.name || '当前版本'}　阶段：{version?.stage || '投拓阶段'}</p><div className="decision" style={{ color: decisionColor }}>{decision}</div><p>基准税后净利 {fmt(base.netProfit)}，净利率 {pct(base.netMargin)}；矩阵亏损格子 {lossCells}/{matrixCells.length}</p></section>
    <section className="block"><h2>一、敏感性结论</h2><ol><li>最差组合：售价{pct(worstCell.price - 1)} / 成本{pct(worstCell.costFactor - 1)}，税后净利 {fmt(worstCell.result.netProfit)}。</li><li>售价安全垫：{factorText(breakEvenPrice, 'down')}；成本安全垫：{factorText(maxCostFactor, 'up')}；土地安全垫：{factorText(maxLandFactor, 'up')}。</li><li>{lossCells > 4 ? '亏损格子较多，项目抗压能力偏弱。' : lossCells > 0 ? '部分压力组合转亏，建议谨慎推进并锁定成本。' : '压力矩阵内未出现亏损格子，具备一定抗压能力。'}</li></ol></section>
    <section className="block"><h2>二、盈亏平衡安全垫</h2><table><tbody><tr><td>售价盈亏平衡</td><td>{breakEvenPrice === null ? '无法测算' : `最低售价 ${pct(breakEvenPrice)}，${factorText(breakEvenPrice, 'down')}`}</td></tr><tr><td>成本盈亏平衡</td><td>{maxCostFactor === null ? '无法测算' : `最高成本 ${pct(maxCostFactor)}，${factorText(maxCostFactor, 'up')}`}</td></tr><tr><td>土地盈亏平衡</td><td>{maxLandFactor === null ? '无法测算' : `最高土地 ${pct(maxLandFactor)}，${factorText(maxLandFactor, 'up')}`}</td></tr></tbody></table></section>
    <section className="block"><h2>三、压力情景测算</h2><table><thead><tr><th>情景</th><th>变化</th><th>税后净利</th><th>净利率</th><th>较基准变化</th></tr></thead><tbody>{scenarios.map((row) => <tr key={row.name}><td>{row.name}</td><td>{row.change}</td><td style={{ color: statusColor(row.result.netProfit), fontWeight: 900 }}>{fmt(row.result.netProfit)}</td><td>{pct(row.result.netMargin)}</td><td style={{ color: statusColor(row.result.netProfit - base.netProfit), fontWeight: 900 }}>{fmt(row.result.netProfit - base.netProfit)}</td></tr>)}</tbody></table></section>
    <section className="block"><h2>四、售价 × 成本二维矩阵</h2><table><thead><tr><th>售价 \ 成本</th>{costFactors.map((factor) => <th key={factor}>成本{pct(factor - 1)}</th>)}</tr></thead><tbody>{matrix.map((row) => <tr key={row.price}><td>售价{pct(row.price - 1)}</td>{row.cells.map((cell) => <td key={`${row.price}-${cell.costFactor}`} style={{ color: statusColor(cell.result.netProfit), fontWeight: 900 }}>{fmt(cell.result.netProfit)}<br />{pct(cell.result.netMargin)}</td>)}</tr>)}</tbody></table></section>
    <section className="block"><h2>五、评审签批区</h2><table><tbody><tr><td>评审结论</td><td>□ 通过　□ 谨慎通过　□ 暂缓</td></tr><tr><td>需复核事项</td><td>□ 售价　□ 建安成本　□ 土地成本　□ 税费　□ 去化周期</td></tr><tr><td>负责人</td><td></td></tr><tr><td>日期</td><td></td></tr><tr><td>备注</td><td style={{ height: 80 }}></td></tr></tbody></table></section>
