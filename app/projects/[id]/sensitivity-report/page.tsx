...
export default async function PrintableSensitivityReport({ params }: { params: { id: string } }) {
  ...
  return <main className="print-report">
    <div className="no-print toolbar"><Link href={`/projects/${project.id}/sensitivity`} className="btn">返回敏感性</Link><Link href={`/projects/${project.id}/decision`} className="btn">投决评审</Link><Link href={`/projects/${project.id}/report-print`} className="btn">打印版经营报告</Link><Link href={`/projects/${project.id}/dashboard-lite`} className="btn">经营总控</Link><span className="btn btn-primary">打印：Ctrl/Cmd + P</span></div>
    <section className="cover block"><div className="eyebrow">源信达地产目标成本测算系统</div><h1>{project.name}</h1><h2>敏感性测算报告</h2><p>当前版本：{version?.name || '当前版本'}　阶段：{version?.stage || '投拓阶段'}</p><div className="decision" style={{ color: decisionColor }}>{decision}</div><p>基准税后净利 {fmt(base.netProfit)} 万元，净利率 {pct(base.netMargin)}；矩阵亏损格子 {lossCells}/{matrixCells.length}</p></section>
    ...
    <section className="block"><h2>五、评审建议</h2><ol><li>若售价下降 5% 或成本上升 5% 后净利明显压缩，应优先锁定售价、去化、建安成本和土地成本。</li><li>若亏损矩阵格子超过 4 个，建议先做方案强排优化和成本压降后再进入投决。</li><li>正式上会前应同步复核税费测算总表、土地增值税清算测算表、业态利润分析。</li></ol></section>
