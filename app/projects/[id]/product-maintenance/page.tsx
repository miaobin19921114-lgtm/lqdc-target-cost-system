...
export default async function ProductMaintenancePage({ params, searchParams }: { params: { id: string }, searchParams?: { saved?: string; duplicate?: string } }) {
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1280 }}>
    <div className="page-header"><div><p className="eyebrow">项目业态维护</p><h1 className="title">{project.name}</h1><p className="subtitle">用于项目创建后补充、减少、调整业态。当前先支持按分类新增模板业态和自定义业态；已有数据的删除/停用下一步接入。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/overview`} className="btn btn-primary">返回概况表</Link><Link href={`/projects/${project.id}`} className="btn">工作台</Link></div></div>
    {searchParams?.saved ? <div className="card" style={{ marginBottom: 14, borderColor: '#b2f2bb' }}>业态已更新。</div> : null}
    {searchParams?.duplicate ? <div className="card" style={{ marginBottom: 14, borderColor: '#ffd8a8' }}>该业态已在本项目中，请在概况表已选业态里修改。</div> : null}
    <section className="card" style={{ marginBottom: 14 }}><h2>本项目当前业态</h2><p className="meta">面积、是否可售、是否参与分摊在项目概况表里维护。</p>{currentGroups.length ? <div style={{ display: 'grid', gap: 12 }}>{currentGroups.map((group) => <div key={group.category} style={{ border: '1px solid #eef2f6', borderRadius: 12, overflow: 'hidden' }}><div style={{ background: '#f8fafc', padding: '10px 12px', fontWeight: 900 }}>{group.category}</div><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 720, fontSize: 13 }}><thead><tr>{['业态', '建筑面积', '计容面积', '可售面积', '销售', '分摊', '状态'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 9, borderBottom: '1px solid #eef2f6', color: '#667085' }}>{head}</th>)}</tr></thead><tbody>{group.items.map((item) => <tr key={item.id}><td style={{ padding: 9, borderBottom: '1px solid #eef2f6', fontWeight: 800 }}>{item.name}</td><td style={{ padding: 9, borderBottom: '1px solid #eef2f6' }}>{numberValue(item.buildingArea)}㎡</td><td style={{ padding: 9, borderBottom: '1px solid #eef2f6' }}>{numberValue(item.capacityArea)}㎡</td><td style={{ padding: 9, borderBottom: '1px solid #eef2f6' }}>{numberValue(item.saleableArea)}㎡</td><td style={{ padding: 9, borderBottom: '1px solid #eef2f6' }}>{item.isSaleable ? '可售' : '不可售'}</td><td style={{ padding: 9, borderBottom: '1px solid #eef2f6' }}>{item.participateAllocation ? '参与' : '不参与'}</td><td style={{ padding: 9, borderBottom: '1px solid #eef2f6' }}>{item.remark?.includes('已停用') ? '停用' : '启用'}</td></tr>)}</tbody></table></div></div>)}</div> : <p className="meta">暂无业态。</p>}</section>
    <section className="card" style={{ marginBottom: 14 }}><h2>从模板补充业态</h2><p className="meta">已在项目中的业态不会出现在下拉里，避免重复。</p><form action={`/api/projects/${project.id}/products`} method="post" style={{ display: 'grid', gridTemplateColumns: '220px 1fr 130px 130px', gap: 10, alignItems: 'end' }}><input type="hidden" name="returnPath" value="product-maintenance" /><input type="hidden" name="mode" value="create" /><label>一级分类<select name="category" style={{ width: '100%', height: 36 }}>{templateGroups.map((group) => <option key={group.category} value={group.category}>{group.category}</option>)}</select></label><label>二级业态<select name="name" style={{ width: '100%', height: 36 }}>{templateGroups.map((group) => <optgroup key={group.category} label={group.category}>{group.items.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</optgroup>)}</select></label><label style={{ display: 'flex', alignItems: 'center', gap: 8, height: 36 }}><input name="isSaleable" type="checkbox" defaultChecked />可售</label><label style={{ display: 'flex', alignItems: 'center', gap: 8, height: 36 }}><input name="participateAllocation" type="checkbox" defaultChecked />分摊</label><button className="btn btn-primary" style={{ gridColumn: '1 / -1' }}>添加到本项目</button></form></section>
