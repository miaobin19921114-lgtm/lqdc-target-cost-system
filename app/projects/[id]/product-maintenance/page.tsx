...
export default async function ProductMaintenancePage({ params, searchParams }: { params: { id: string }, searchParams?: Record<string, string | undefined> }) {
  ...
  const liquidationRows = version ? await prisma.$queryRawUnsafe<Array<{ id: string; taxLiquidationObject: string | null }>>('SELECT "id", "taxLiquidationObject" FROM "ProductType" WHERE "projectVersionId" = $1', version.id) : [];
  const liquidationMap = new Map(liquidationRows.map((row) => [row.id, row.taxLiquidationObject]));
  ...
  return <main className="page" style={{ background: '#eef3f8' }}><div className="container" style={{ maxWidth: 1520 }}>
    ...
    <section className="card" style={{ marginBottom: 14, borderColor: '#d0ebff', background: '#f8fbff' }}><b>税务清算对象说明</b><p className="meta" style={{ margin: '6px 0 0' }}>住宅按普通住宅≤140㎡、非普通住宅＞140㎡拆分；商业、车位、办公公寓等按非住宅对象拆分。土地增值税清算测算表按该字段汇总，不再只靠业态名称临时判断。</p></section>
    ...
    <section className="card" style={{ marginBottom: 14 }}><h2>本项目当前业态</h2><p className="meta">“税务清算对象”影响土增税清算分组；“成本测算设置”影响专业明细表的分组展示和归属。</p>{currentGroups.length ? <div style={{ display: 'grid', gap: 12 }}>{currentGroups.map((group) => <div key={group.category} style={{ border: '1px solid #eef2f6', borderRadius: 12, overflow: 'hidden' }}><div style={{ background: '#f8fafc', padding: '10px 12px', fontWeight: 900 }}>{group.category}</div><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1780, fontSize: 13 }}><thead><tr>{['业态', '建筑面积', '可售面积', '销售属性', '分摊属性', '税务清算对象', '成本测算设置', '历史数据', '当前状态', '处理建议', '操作'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 9, borderBottom: '1px solid #eef2f6', color: '#667085' }}>{head}</th>)}</tr></thead><tbody>{group.items.map((item) => {
