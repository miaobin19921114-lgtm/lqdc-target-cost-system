...
export default async function ProductMaintenancePage({ params, searchParams }: { params: { id: string }, searchParams?: Record<string, string | undefined> }) {
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1320 }}>
    <div className="page-header"><div><p className="eyebrow">项目业态维护</p><h1 className="title">{project.name}</h1><p className="subtitle">用于项目创建后补充、停用、恢复或删除业态。已有收入或成本数据的业态建议恢复，不建议硬删除。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/overview`} className="btn btn-primary">返回概况表</Link><Link href={`/projects/${project.id}`} className="btn">工作台</Link></div></div>
    <StatusMessage searchParams={searchParams} />
    <div className="summary-strip" style={{ marginBottom: 14 }}><div className="stat"><div className="stat-label">启用业态</div><div className="stat-value">{activeCount}</div></div><div className="stat"><div className="stat-label">停用业态</div><div className="stat-value">{disabledCount}</div></div><div className="stat"><div className="stat-label">有历史数据</div><div className="stat-value">{withHistoryCount}</div></div><div className="stat"><div className="stat-label">模板可补充</div><div className="stat-value">{templateGroups.reduce((sum, group) => sum + group.items.length, 0)}</div></div></div>
    <section className="card" style={{ marginBottom: 14, borderColor: '#d0ebff', background: '#f8fbff' }}><b>操作规则</b><p className="meta" style={{ margin: '6px 0 0' }}>无历史收入/成本的业态可以直接删除；已有历史数据的业态只能停用或恢复。恢复时系统会自动恢复“参与分摊”，有可售面积或收入记录的业态会同步恢复为可售。</p></section>
