...
export default async function ProfitAnalysisPage({ params }: { params: { id: string } }) {
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1280 }}>
    ...
    <div className="summary-strip"><div className="stat"><div className="stat-label">不含税销售收入</div><div className="stat-value">{fmt(revenue.taxExclusive)}</div></div><div className="stat"><div className="stat-label">开发成本及费用</div><div className="stat-value">{fmt(cost.taxExclusive)}</div></div><div className="stat"><div className="stat-label">税前经营利润</div><div className="stat-value">{fmt(formalProfitBeforeIncomeTax)}</div></div><div className="stat"><div className="stat-label">土地增值税</div><div className="stat-value">{fmt(formalLandVat)}</div></div><div className="stat"><div className="stat-label">税后净利</div><div className="stat-value">{fmt(netProfit)}</div></div><div className="stat"><div className="stat-label">销售净利率</div><div className="stat-value" style={{ color: color(netMargin) }}>{pct(netMargin)}</div></div></div>
    <section className="card" style={{ marginTop: 16 }}><h2>数据状态</h2><div className="summary-strip"><div className="stat"><div className="stat-label">参与利润业态</div><div className="stat-value">{saleableProducts.length}</div></div><div className="stat"><div className="stat-label">仅成本对象</div><div className="stat-value">{activeProducts.filter((item) => !item.isSaleable).length}</div></div><div className="stat"><div className="stat-label">收入待补业态</div><div className="stat-value">{productRows.filter((row) => row.product.isSaleable && row.revenueInclusive <= 0).length}</div></div><div className="stat"><div className="stat-label">成本是否完整</div><div className="stat-value">{effective.effective.length ? '临时测算' : '待生成'}</div></div></div></section>
    ...
    <section className="card" style={{ marginTop: 16 }}><h2>业态利润明细</h2>{productRows.length ? <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 1760, borderCollapse: 'collapse', fontSize: 12 }}><thead><tr>{['业态', '成本归属组', '清算对象', '不含税收入', '不含税成本', '毛利率', '土地增值税', '税后净利', '净利率', '建面单方利润', '可售单方利润', '状态', '操作'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 9, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{head}</th>)}</tr></thead><tbody>{productRows.map((row) => <tr key={row.product.id}><td style={{ ...cell, fontWeight: 800 }}>{row.product.name}</td><td style={cell}>{row.costGroup}</td><td style={cell}>{row.liquidationObject}</td><td style={money}>{fmt(row.revenueExclusive)}</td><td style={money}>{fmt(row.costExclusive)}</td><td style={cell}>{pct(row.revenueExclusive ? (row.revenueExclusive - row.costExclusive) / row.revenueExclusive : 0)}</td><td style={money}>{fmt(row.landVat)}</td><td style={{ ...money, fontWeight: 900, color: color(row.netProfit) }}>{fmt(row.netProfit)}</td><td style={cell}>{pct(row.revenueInclusive ? row.netProfit / row.revenueInclusive : 0)}</td><td style={money}>{fmt(n(row.product.buildingArea) ? row.netProfit * 10000 / n(row.product.buildingArea) : 0)}</td><td style={money}>{fmt(n(row.product.saleableArea) ? row.netProfit * 10000 / n(row.product.saleableArea) : 0)}</td><td style={cell}>{row.revenueInclusive > 0 && row.costExclusive > 0 ? '已形成测算' : '收入或成本待补'}</td><td style={cell}><Link href={`/projects/${project.id}/allocation`} className="btn">查看分摊</Link></td></tr>)}</tbody></table></div> : <EmptyState title="暂无可分析的业态利润">请先维护启用业态、收入和有效成本明细；形成数据后本页会按业态展示利润贡献。</EmptyState>}</section>
