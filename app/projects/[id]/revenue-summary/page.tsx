...
export default async function RevenueSummaryPage({ params }: { params: { id: string } }) {
  ...
  return <main className="page" style={{ background: '#eef3f8' }}><div className="container" style={{ maxWidth: 1280 }}>
    <div className="page-header"><div><p className="eyebrow">收入测算</p><h1 className="title">收入汇总</h1><p className="subtitle">统一查看销售收入、商业专项收入、车位收入、其他政策性收入，不在本页直接录入数据。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/revenue`} className="btn">销售收入测算</Link><Link href={`/projects/${project.id}/commercial-revenue`} className="btn">商业收入</Link><Link href={`/projects/${project.id}/parking-revenue`} className="btn">车位收入</Link><Link href={`/projects/${project.id}/other-revenue`} className="btn">其他收入</Link><Link href={`/projects/${project.id}/dashboard-lite`} className="btn btn-primary">经营总控</Link></div></div>
    ...
    <section className="card" style={{ borderColor: chargingRevenueRows ? '#ffc9c9' : '#b2f2bb', background: chargingRevenueRows ? '#fff5f5' : '#f0fff4' }}><h2>收入口径校验</h2><p className="meta">收入汇总只负责看总数；销售收入、商业收入、车位收入、其他收入分别在专项页面维护。</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 }}><div><b>充电桩收入行</b><p className="meta">疑似充电桩收入行：{chargingRevenueRows} 条。充电桩不作为收入业态，应作为配置指标或设备/安装成本。</p></div><div><b>总收入构成</b><p className="meta">销售收入 + 商业专项收入 + 车位收入 + 其他收入已统一进入经营总控和税金明细。</p></div></div></section>
  </div></main>;
}
