...
export default async function OtherRevenuePage({ params, searchParams }: { params: { id: string }, searchParams?: { saved?: string; rows?: string; locked?: string } }) {
  ...
  const rows = incomeTypes.map((item) => {
    const product = products.find((p) => p.name === `其他收入-${item.name}`);
    const revenue = product ? revenueMap.get(product.id) : null;
    const amount = Number(product?.salePrice || 0);
    const taxRate = Number(revenue?.taxRate || 0);
    ...
    return {
      ...
      maintained: revenue ? Number(revenue.taxInclusiveRevenue || 0) : 0,
      cashDate: readRemark(product?.remark, '兑现时间'),
      certainty: readRemark(product?.remark, '确定性') || '待确认',
      condition: readRemark(product?.remark, '兑现条件'),
      remark: readRemark(product?.remark, '备注')
    };
  });
  ...
  const maintained = activeRows.reduce((sum, row) => sum + row.maintained, 0);
  ...
  return <main className="page" style={{ background: '#eef3f8' }}><div className="container" style={{ maxWidth: 1280 }}>
    <div className="page-header"><div><p className="eyebrow">收入测算</p><h1 className="title">其他收入测算</h1><p className="subtitle">税收返还、产业奖励、财政补贴、土地款返还等政策性收益单独测算，不与住宅、商业、车位销售收入混算。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/revenue`} className="btn">收入明细</Link><Link href={`/projects/${project.id}/parking-revenue`} className="btn">车位收入</Link><Link href={`/projects/${project.id}/indicator-check`} className="btn">指标校验中心</Link><Link href={`/projects/${project.id}`} className="btn">项目测算中心</Link></div></div>
    {searchParams?.saved === '1' ? <div className="card" style={{ marginBottom: 16, borderColor: '#b2f2bb', background: '#f0fff4' }}>其他收入已保存并同步。{searchParams?.rows ? `本次处理 ${searchParams.rows} 行。` : ''}</div> : null}
