...
export default async function ProjectCheckPage({ params }: { params: { id: string } }) {
  ...
  const checks: CheckItem[] = [
    ...
    },
    {
      ...
      item: '项目指标校验分流',
      status: '通过',
      detail: '项目概况、业态面积、收入、成本、税费勾稽请在 V1 主流程页面逐项复核。',
      href: undefined
    }
  ];
  ...
  return <main className="page" style={{ background: '#eef3f8' }}><div className="container" style={{ maxWidth: 1280 }}>
    <div className="page-header"><div><p className="eyebrow">导入与配置</p><h1 className="title">系统自检</h1><p className="subtitle">检查系统运行状态、导入配置和 V1 功能边界。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}`} className="btn btn-primary">项目测算中心</Link></div></div>
    ...
    <section className="card" style={{ marginBottom: 14, borderColor: '#c5eef3', background: '#f8fbff' }}><b>自检口径说明</b><p className="meta" style={{ margin: '6px 0 0' }}>系统自检只看系统底座和导入配置，不判断当前项目测算是否完整；项目业务勾稽请在 V1 主流程页面逐项复核。</p></section>
