    ...
    <main style={{ minHeight: '100vh', background: '#eef3f8', color: '#102033' }}>
      ...
      <div className="sys-shell" style={{ display: 'grid', gridTemplateColumns: '260px 1fr 300px', gap: 12, padding: 12 }}>
        <aside style={{ background: '#fff', border: '1px solid #d9e2ec', borderRadius: 10, overflow: 'hidden' }}>
          ...
          <div style={{ padding: 10 }}>{projectNavGroups.map((group) => <div key={group.title} style={{ marginBottom: 10 }}><div style={{ fontSize: 12, color: '#667085', fontWeight: 800, padding: 8 }}>{group.title}</div>{group.items.map(([name, href]) => <Link key={`${group.title}-${name}`} href={`/projects/${project.id}/${href}`} style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 10px', borderRadius: 8, fontSize: 14 }}><span>{name}</span><span style={{ color: '#0b7285' }}>›</span></Link>)}</div>)}</div>
        </aside>
