  ...
  return (
    <main style={{ minHeight: '100vh', background: '#eef3f8', color: '#102033' }}>
      <div style={{ height: 52, background: '#12384b', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 18px' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}><Link href={`/projects/${project.id}/profit-analysis`} className="btn" style={{ minHeight: 34, background: '#fff', color: '#12384b' }}>业态利润</Link><Link href={`/projects/${project.id}/cost-mapping`} className="btn" style={{ minHeight: 34, background: '#fff', color: '#12384b' }}>科目映射</Link><Link href={`/projects/${project.id}/export`} className="btn" style={{ minHeight: 34, background: '#fff', color: '#12384b' }}>导入/导出</Link><Link href="/projects" className="btn" style={{ minHeight: 34, color: '#fff', background: 'transparent', borderColor: 'rgba(255,255,255,.35)' }}>项目列表</Link></div>
      </div>
