...
export default async function RevenuePage({ params, searchParams }: { params: { id: string }, searchParams?: { saved?: string; synced?: string; rows?: string; locked?: string } }) {
  ...
  const ordinaryProducts = allProducts.filter((item) => item.isActive && item.isSaleable && !isParkingProduct(item.name) && !isChargingProduct(item.name));
  ...
  const total = ordinaryTotal + parkingTotal;
  const net = ordinaryNet + parkingNet;
  const fee = ordinaryFee + parkingFee;
  ...
  return <main className="page" style={{ background: '#eef3f8' }}><div className="container" style={{ maxWidth: 1280 }}>
    <div className="page-header"><div><p className="eyebrow">收入测算</p><h1 className="title">收入明细表</h1><p className="subtitle">普通业态按“可售面积×含税销售单价”测算；车位收入按“个数×单个车位含税单价”在车位收入测算页专项维护。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/overview`} className="btn">项目概况</Link><Link href={`/projects/${project.id}/parking-revenue`} className="btn">车位收入测算</Link><Link href={`/projects/${project.id}/indicator-check`} className="btn">指标校验中心</Link><Link href={`/projects/${project.id}`} className="btn">项目测算中心</Link></div></div>
    ...
    <div className="summary-strip" style={{ marginBottom: 14 }}><div className="stat"><div className="stat-label">普通可售面积</div><div className="stat-value">{fmt(totalArea)}㎡</div></div><div className="stat"><div className="stat-label">普通业态含税收入</div><div className="stat-value">{fmt(ordinaryTotal)}元</div></div><div className="stat"><div className="stat-label">车位含税收入</div><div className="stat-value">{fmt(parkingTotal)}元</div></div><div className="stat"><div className="stat-label">总含税收入</div><div className="stat-value">{fmt(total)}元</div></div><div className="stat"><div className="stat-label">销项税额</div><div className="stat-value">{fmt(fee)}元</div></div></div>

    <section className="card" style={{ marginBottom: 16 }}><h2>收入汇总</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}><div><span className="meta">总含税销售收入</span><div style={{ fontWeight: 900, fontSize: 20 }}>{fmt(total)}</div></div><div><span className="meta">总不含税销售收入</span><div style={{ fontWeight: 900, fontSize: 20 }}>{fmt(net)}</div></div><div><span className="meta">总销项税额</span><div style={{ fontWeight: 900, fontSize: 20 }}>{fmt(fee)}</div></div><div><span className="meta">普通业态可售单方</span><div style={{ fontWeight: 900, fontSize: 20 }}>{totalArea > 0 ? fmt(ordinaryTotal / totalArea) : '0'}元/㎡</div></div></div></section>

    <section className="card" style={{ marginBottom: 16 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', marginBottom: 12 }}><div><h2 style={{ margin: 0 }}>普通业态收入</h2><p className="meta">住宅、商业、配套等普通业态按可售面积 × 含税销售单价。车位和充电桩不在这里混算。</p></div><button form="revenue-batch" className="btn btn-primary">保存普通业态单价并同步</button></div>
      {rows.length === 0 ? <p className="meta">暂无普通可售业态。请先到项目概况维护业态，车位收入请到“车位收入测算”。</p> : <div style={{ overflowX: 'auto' }}><form id="revenue-batch" action={`/api/projects/${project.id}/revenue/batch`} method="post" /><input form="revenue-batch" type="hidden" name="rowCount" value={rows.length} /><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1080 }}><thead><tr>{['业态', '可售面积㎡', '含税销售单价', '税率', '含税收入', '不含税收入', '销项税额', '已同步收入', '差异'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 10, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{head}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={row.id}><td style={{ padding: 10, borderBottom: '1px solid var(--border)', fontWeight: 700 }}><input form="revenue-batch" type="hidden" name={`productId-${index}`} value={row.id} />{row.name}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.area)}</td><td style={{ padding: 6, borderBottom: '1px solid var(--border)' }}><input form="revenue-batch" name={`salePrice-${index}`} type="number" step="0.01" defaultValue={row.price || ''} style={{ height: 34, border: '1px solid #d9e2ec', borderRadius: 6, padding: '4px 8px', width: 140 }} /></td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{(taxRate * 100).toFixed(2)}%</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.total)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.net)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.fee)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(row.maintainedTotal)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)', color: Math.abs(row.diff) > 1 ? '#e03131' : '#2f9e44', fontWeight: 900 }}>{fmt(row.diff)}</td></tr>)}</tbody></table></div>}
    </section>

    <section className="card" style={{ marginBottom: 16, borderColor: '#d0ebff', background: '#f8fbff' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}><div><h2 style={{ margin: 0 }}>车位收入提示</h2><p className="meta" style={{ margin: '6px 0 0' }}>车位产品 {parkingProducts.length} 个，已同步车位收入行 {parkingRevenueLines.length} 条，车位含税收入 {fmt(parkingTotal)} 元。车位收入按个数 × 单个车位含税销售单价专项维护。</p></div><Link href={`/projects/${project.id}/parking-revenue`} className="btn btn-primary">进入车位收入测算</Link></div></section>
