...
export default async function PrintableOperatingReport({ params }: { params: { id: string } }) {
  ...
  return <main className="print-report">
    <div className="no-print toolbar"><Link href={`/projects/${project.id}/report`} className="btn">返回经营报告</Link><Link href={`/projects/${project.id}/dashboard-lite`} className="btn">经营总控</Link><Link href={`/projects/${project.id}/sensitivity-report`} className="btn">敏感性报告</Link><span className="btn btn-primary">打印：Ctrl/Cmd + P</span></div>
    <section className="cover block"><div className="eyebrow">源信达地产目标成本测算系统</div><h1>{project.name}</h1><h2>项目经营测算报告</h2><p>当前版本：{version?.name || '当前版本'}　阶段：{version?.stage || '投拓阶段'}　城市/区域：{project.city || '-'} / {project.district || '-'}</p><div className="decision" style={{ color: decision.color }}>{decision.level}</div><p>税后净利 {fmt(tax.netProfit)}，销售净利率 {pct(netMargin)}</p></section>
    <section className="block"><h2>一、经营结论</h2><ol><li>{decision.reason}</li><li>含税收入 {fmt(revenue.taxInclusive)}，含税目标成本 {fmt(cost.taxInclusive)}，税费合计 {fmt(tax.totalTax)}。</li><li>毛利率 {pct(grossMargin)}，税后净利率 {pct(netMargin)}，综合税负率 {pct(taxBurden)}。</li><li>下一步动作：{decision.action}</li></ol></section>
    ...
    <section className="block"><h2>三、压力测算摘要</h2><table><thead><tr><th>情景</th><th>税后净利</th><th>净利率</th></tr></thead><tbody><tr><td>售价下降5%</td><td style={{ color: statusColor(priceDown5.netProfit), fontWeight: 900 }}>{fmt(priceDown5.netProfit)}</td><td>{pct(priceDown5.netMargin)}</td></tr><tr><td>成本上升5%</td><td style={{ color: statusColor(costUp5.netProfit), fontWeight: 900 }}>{fmt(costUp5.netProfit)}</td><td>{pct(costUp5.netMargin)}</td></tr><tr><td>双重压力</td><td style={{ color: statusColor(doublePressure.netProfit), fontWeight: 900 }}>{fmt(doublePressure.netProfit)}</td><td>{pct(doublePressure.netMargin)}</td></tr><tr><td>最差矩阵组合</td><td style={{ color: statusColor(worstCell.result.netProfit), fontWeight: 900 }}>{fmt(worstCell.result.netProfit)}</td><td>{pct(worstCell.result.netMargin)}</td></tr></tbody></table></section>
    <section className="block"><h2>四、风险提示</h2><table><thead><tr><th>风险项</th><th>等级</th><th>说明</th></tr></thead><tbody>{risks.map((risk) => <tr key={risk.name}><td>{risk.name}</td><td style={{ color: riskColor(risk.level), fontWeight: 900 }}>{risk.level}</td><td>{risk.text}</td></tr>)}</tbody></table></section>
    <section className="block"><h2>五、复核签批区</h2><table><tbody><tr><td>评审结论</td><td>□ 通过　□ 谨慎通过　□ 暂缓</td></tr><tr><td>需复核事项</td><td>□ 售价　□ 建安成本　□ 土地成本　□ 税费　□ 去化周期</td></tr><tr><td>负责人</td><td></td></tr><tr><td>日期</td><td></td></tr><tr><td>备注</td><td style={{ height: 80 }}></td></tr></tbody></table></section>
