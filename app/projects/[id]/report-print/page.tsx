...
export default async function PrintableOperatingReport({ params }: { params: { id: string } }) {
  ...
  return <main className="print-report">
    <div className="no-print toolbar"><Link href={`/projects/${project.id}/report`} className="btn">经营报告</Link><Link href={`/projects/${project.id}/dashboard-lite`} className="btn">经营总控</Link><Link href={`/projects/${project.id}/tax-details`} className="btn">税费总表</Link><span className="btn btn-primary">打印：Ctrl/Cmd + P</span></div>
    ...
    <section className="block"><h2>四、正式明细入口</h2><table><tbody><tr><td>税费测算总表</td><td>增值税、附加税、土增税、企业所得税</td></tr><tr><td>土地增值税清算测算表</td><td>按清算对象复核扣除项目、增值率、适用税率</td></tr><tr><td>业态利润分析</td><td>按业态查看收入、成本、税后利润和净利率</td></tr></tbody></table></section>
