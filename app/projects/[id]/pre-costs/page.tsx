      ...
      <div className="container" style={{ maxWidth: 1360 }}>
        <div className="page-header">
          <div>
            ...
            <p className="subtitle">一二三四级科目、区域/业态、税率、测算依据、分摊口径均从成本科目词典预设。</p>
          </div>
        ...
        <section className="form-card" style={{ maxWidth: '100%', marginBottom: 18 }}>
          <h2>新增前期费用</h2>
          <p className="meta">选择成本科目后，系统自动带出末级科目、区域/业态、测算依据、单位、默认税率、分摊口径。确需调整时再手动覆盖。</p>
        ...
        <section className="card">
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', minWidth: 1380, borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr>{['成本编码', '科目路径', '费用项目', '区域/业态', '测算依据', '工程量', '单位', '税率', '含税单价', '含税金额', '不含税金额', '税额', '分摊方式'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 10, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{head}</th>)}</tr></thead>
              <tbody>
                {costs.length === 0 ? <tr><td colSpan={13} style={{ padding: 12, color: 'var(--muted)' }}>暂无前期费用。</td></tr> : costs.map((row) => (
                  <tr key={row.id}>
