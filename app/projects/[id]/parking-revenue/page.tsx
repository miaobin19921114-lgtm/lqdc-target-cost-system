...
export default async function ParkingRevenuePage({ params, searchParams }: { params: { id: string }, searchParams?: { saved?: string; rows?: string; locked?: string } }) {
  ...
  const parkingCountDiff = Number(project.parkingCount || 0) - classifiedParkingCount;
  ...
  return <main className="page" style={{ background: '#eef3f8' }}><div className="container" style={{ maxWidth: 1280 }}>
    ...
    <section className="card"><h2>车位收入校验</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 }}><div style={{ border: '1px solid #eef2f6', borderRadius: 10, padding: 12 }}><b>总数与分类</b><p className="meta">项目总车位 {fmt(project.parkingCount)} 个，分类合计 {fmt(classifiedParkingCount)} 个，差异 {fmt(parkingCountDiff)} 个。</p><strong style={{ color: Math.abs(parkingCountDiff) <= 2 ? '#2f9e44' : '#e03131' }}>{Math.abs(parkingCountDiff) <= 2 ? '正常' : '需复核'}</strong></div><div style={{ border: '1px solid #eef2f6', borderRadius: 10, padding: 12 }}><b>单价完整性</b><p className="meta">数量大于0但单价为0：{zeroPriceRows} 行。</p><strong style={{ color: zeroPriceRows === 0 ? '#2f9e44' : '#f08c00' }}>{zeroPriceRows === 0 ? '正常' : '待补充'}</strong></div><div style={{ border: '1px solid #eef2f6', borderRadius: 10, padding: 12 }}><b>充电桩口径</b><p className="meta">充电桩 {chargingPileCount} 个，车位配置比例约 {pct(chargingRatio)}。不作为收入业态。</p><strong style={{ color: '#2f9e44' }}>正常</strong></div><div style={{ border: '1px solid #eef2f6', borderRadius: 10, padding: 12 }}><b>同步状态</b><p className="meta">自动测算与已同步收入差异：{fmt(diffTotal)} 元。</p><strong style={{ color: Math.abs(diffTotal) > 1 ? '#e03131' : '#2f9e44' }}>{Math.abs(diffTotal) > 1 ? '需同步' : '正常'}</strong></div></div></section>
  </div></main>;
}
