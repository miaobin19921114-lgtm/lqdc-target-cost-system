...
export default async function TargetCostBatchPage({ params, searchParams }: { params: { id: string }, searchParams?: { saved?: string, batch?: string } }) {
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1700 }}>
    <div className="page-header"><div><p className="eyebrow">目标成本编制</p><h1 className="title">{project.name}</h1><p className="subtitle">树状科目：一级 / 二级 / 三级为汇总节点，四级末级科目才允许录入工程量、含税单价、备注。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/summary`} className="btn btn-primary">汇总表</Link><Link href={`/projects/${project.id}/costs`} className="btn">旧目标成本页</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
                  ...
                  <div style={{ padding: 8 }}>
                    {Array.from(level2.children.values()).map((level3) => (
                      <details key={level3.name} open style={{ border: '1px solid #eef2f6', borderRadius: 8, marginBottom: 8, overflow: 'hidden' }}>
                        ...
                        <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 1600, borderCollapse: 'collapse', fontSize: 12 }}><thead><tr style={{ background: '#fff' }}>{['编码', '末级科目', '业态', '测算依据', '工程量', '单位', '含税单价', '税率', '含税金额', '建面单方', '可售单方', '分摊方式', '备注', '状态'].map((head) => <th key={head} style={{ ...cell, textAlign: 'left', color: '#475467' }}>{head}</th>)}</tr></thead><tbody>{level3.leaves.map((row, index) => {
                          const saved = row.costCode ? byCode.get(row.costCode) : null;
                          const amount = Number(saved?.taxInclusiveAmount || 0);
