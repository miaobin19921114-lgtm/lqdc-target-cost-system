...
export default async function TargetCostBatchPage({ params, searchParams }: { params: { id: string }, searchParams?: { saved?: string, batch?: string } }) {
  ...
  const rows = await prisma.costDictionaryRow.findMany({
    where: { projectId: params.id, enabled: { not: '否' }, costCode: { not: null }, OR: [{ detailSubject: { not: null } }, { subjectLevel: { in: ['3', '4'] } }] },
  ...
  const total = costs.reduce((sum, item) => sum + Number(item.taxInclusiveAmount || 0), 0);
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1700 }}>
    <div className="page-header"><div><p className="eyebrow">目标成本编制</p><h1 className="title">{project.name}</h1><p className="subtitle">科目预设、左侧冻结、一级汇总、整表批量保存；只填工程量、含税单价、备注。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/summary`} className="btn btn-primary">汇总表</Link><Link href={`/projects/${project.id}/costs`} className="btn">旧目标成本页</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
    ...
    <div className="summary-strip"><div className="stat"><div className="stat-label">含税目标成本</div><div className="stat-value">{fmt(total)}</div></div><div className="stat"><div className="stat-label">建面单方</div><div className="stat-value">{fmt(single(total, buildingArea))}</div></div><div className="stat"><div className="stat-label">可售单方</div><div className="stat-value">{fmt(single(total, saleableArea))}</div></div><div className="stat"><div className="stat-label">预设行</div><div className="stat-value">{rows.length}</div></div></div>
    <section className="card" style={{ padding: 0, overflow: 'hidden' }}><div style={{ padding: 12, borderBottom: '1px solid var(--border)', background: '#f8fafc', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><b>目标成本测算表｜批量填报模式</b><button form="target-cost-batch" className="btn btn-primary">整表批量保存</button></div><form id="target-cost-batch" action={`/api/projects/${project.id}/costs/batch`} method="post" />
