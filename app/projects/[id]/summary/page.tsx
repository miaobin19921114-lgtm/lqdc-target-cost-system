...
export default async function TargetCostSummaryPage({ params }: { params: { id: string } }) {
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1380 }}>
    ...
    {disabledProductCount > 0 ? <div className="card" style={{ marginBottom: 12, borderColor: '#ffd8a8', background: '#fff9db' }}>本项目当前版本有 {disabledProductCount} 个停用业态，收入测算仍按启用业态口径展示。</div> : null}
    {summaryRows.length === 0 ? <div className="card" style={{ marginBottom: 12, borderColor: '#ffd8a8', background: '#fff9db' }}>暂无目标成本汇总聚合数据。请先进入“目标成本测算表”，点击“从明细结果刷新”。</div> : null}
