...
export default async function TargetCostSummaryPage({ params }: { params: { id: string } }) {
  ...
  const version = await prisma.projectVersion.findFirst({
    ...
    include: { products: true, taxes: true, costs: { include: { costSubject: true, productType: true } } }
  });
  ...
  const revenue = revenueFromProducts(allProducts, vatRate);
  ...
  const metrics: Array<[string, number, 'money' | 'percent']> = [
    ['销售收入（含税）', revenue.taxInclusive, 'money'], ['销售收入（不含税）', revenue.taxExclusive, 'money'], ['开发成本及费用合计（含税，末级）', cost.taxInclusive, 'money'],
    ['开发成本及费用合计（不含税，末级）', cost.taxExclusive, 'money'], ['销项税额', revenue.outputVat, 'money'], ['进项税额（末级）', cost.inputVat, 'money'],
    ['应缴增值税', tax.payableVat, 'money'], ['附加税费', tax.surcharge, 'money'], ['土地增值税', tax.landVat.landVat, 'money'], ['税前经营利润', tax.profitBeforeIncomeTax, 'money'],
    ['税前销售利润率', preTaxMargin, 'percent'], [`所得税（${pct(incomeTaxRate)}）`, tax.incomeTax, 'money'], ['税后净利', tax.netProfit, 'money'], ['销售净利率', netMargin, 'percent']
  ];

  return (
    <main className="page"><div className="container" style={{ maxWidth: 1380 }}>
      <div className="page-header"><div><p className="eyebrow">目标成本汇总表</p><h1 className="title">{project.name}</h1><p className="subtitle">当前汇总基于当前启用版本，统计启用业态和末级成本行；Excel导入四级科目会计入汇总；土增税和所得税已联动税金明细口径。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/revenue`} className="btn">收入明细</Link><Link href={`/projects/${project.id}/tax-details`} className="btn">税金明细</Link><Link href={`/projects/${project.id}/summary-check`} className="btn">汇总联动校验</Link><Link href={`/projects/${project.id}/costs-batch`} className="btn btn-primary">目标成本编制</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
      ...
      <div className="summary-strip"><div className="stat"><div className="stat-label">含税销售收入</div><div className="stat-value">{fmt(revenue.taxInclusive)}元</div></div><div className="stat"><div className="stat-label">末级含税目标成本</div><div className="stat-value">{fmt(cost.taxInclusive)}元</div></div><div className="stat"><div className="stat-label">建面单方 / 可售单方</div><div className="stat-value">{fmt(buildingUnitCost)} / {fmt(saleableUnitCost)}</div></div><div className="stat"><div className="stat-label">税后净利率</div><div className="stat-value">{pct(netMargin)}</div></div></div>
