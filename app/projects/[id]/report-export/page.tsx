...
export default async function ReportExportPage({ params }: { params: { id: string } }) {
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1180 }}>
    <div className="page-header"><div><p className="eyebrow">PDF / Word 导出中心</p><h1 className="title">{project.name}</h1><p className="subtitle">先把报告页面排版稳定，现阶段用浏览器打印/另存 PDF；Word 导出后续再接 docx 模板。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/boss-report`} className="btn btn-primary">老板汇报版</Link><Link href={`/projects/${project.id}/report`} className="btn">经营报告</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
