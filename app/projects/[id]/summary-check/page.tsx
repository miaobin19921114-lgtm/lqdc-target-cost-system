...
export default async function SummaryCheckPage({ params }: { params: { id: string } }) {
  ...
  const costs = allCosts.filter((item) => !item.productTypeId || item.productType?.isActive);
  const disabledCostRows = allCosts.length - costs.length;
  const allRevenues = version?.revenues || [];
  const revenues = allRevenues.filter((item) => item.productType?.isActive && item.productType?.isSaleable);
  const disabledRevenueRows = allRevenues.length - revenues.length;
