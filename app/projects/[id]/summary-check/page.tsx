...
export default async function SummaryCheckPage({ params }: { params: { id: string } }) {
  ...
  const checks = [
    ...
    { name: '税金参数', level: version?.taxes ? 'ok' : 'warn', note: version?.taxes ? `增值税率：${fmt(n(version.taxes.vatRate) * 100)}%` : '未配置税金参数，使用默认税率口径' },
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1180 }}>
    <div className="page-header"><div><p className="eyebrow">汇总联动校验</p><h1 className="title">{project.name}</h1><p className="subtitle">检查概况、业态、收入、成本、税金与汇总口径是否具备联动计算基础。</p></div><div className="actions" style={{ marginTop: 0 }}><Link className="btn btn-primary" href={`/projects/${project.id}/summary`}>目标成本汇总表</Link><Link className="btn" href={`/projects/${project.id}/revenue`}>收入明细</Link><Link className="btn" href={`/projects/${project.id}/tax-details`}>税金明细</Link><Link className="btn" href={`/projects/${project.id}`}>返回工作台</Link></div></div>
