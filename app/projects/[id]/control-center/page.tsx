...
export default async function ControlCenterPage({ params }: { params: { id: string } }) {
  ...
  return <main className="page"><div className="container">
    <div className="page-header"><div><p className="eyebrow">测算控制中心</p><h1 className="title">{project.name}</h1><p className="subtitle">用于核对控制台数据口径：收入取可售业态，成本只取末级有效成本行。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link><Link href={`/projects/${project.id}/summary`} className="btn btn-primary">汇总表</Link></div></div>
