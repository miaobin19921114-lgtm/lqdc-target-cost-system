...
export default async function TaxDetailsPage({ params }: { params: { id: string } }) {
  ...
  effective.effective.forEach((row) => {
    const method = readProjectAllocationMethod(row.costSubject.code, row.allocationMethod, ruleMap, 'incomeTax') || '按可售面积占比';
  ...
  const objectRows = Array.from(objectMap.values()).filter((item) => item.product.isSaleable || item.revenueInclusive || item.costExclusive).map((item) => {
    ...
    return { ...item, payableVat, surcharge, landVat: lv.landVat, taxableIncome, incomeTax, totalTax: payableVat + surcharge + lv.landVat + incomeTax, netProfit: taxableIncome - incomeTax, liquidationObject };
  });
  ...
  const rows: Row[] = [['含税总收入', revenue.taxInclusive, '销售收入+商业专项收入+车位收入+其他收入'], ['不含税总收入', revenue.taxExclusive, '按各收入行税率拆分后的不含税收入'], ['销项税额', revenue.outputVat, '按各收入行税额汇总'], ['进项税额', cost.inputVat, '有效末级成本税额汇总'], ['应缴增值税', quickTax.payableVat, '销项税额-进项税额，低于0按0暂估'], ['附加税费', quickTax.surcharge, `应缴增值税×${fmt(surchargeRate * 100)}%`], ['土地增值税', formalLandVat, '引用土地增值税清算测算表：按清算对象分别计算后汇总'], ['企业所得税应纳税所得额', formalProfitBeforeIncomeTax, '不含税收入-不含税成本-附加税费-正式土增税'], ['企业所得税', formalIncomeTax, `应纳税所得额×${fmt(incomeTaxRate * 100)}%`], ['税费合计', formalTotalTax, '增值税+附加税+正式土增税+所得税']];

  return <main className="page"><div className="container" style={{ maxWidth: 1400 }}>
    <div className="page-header"><div><p className="eyebrow">税费测算总表</p><h1 className="title">{project.name}</h1><p className="subtitle">金额单位统一为万元；业态成本对象拆分优先读取模板中心的“所得税成本对象规则”。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/land-vat`} className="btn btn-primary">土地增值税清算</Link><Link href={`/projects/${project.id}/profit-analysis`} className="btn">业态利润</Link><Link href={`/projects/${project.id}/summary`} className="btn">目标成本汇总</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
