...
export default async function TaxDetailsPage({ params }: { params: { id: string } }) {
  ...
  const rows = [
    ['销项税额', outputVat, '当前版本启用且可售业态收入明细表税额汇总'],
    ['进项税额', inputVat, '当前版本启用业态成本明细税额汇总'],
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1180 }}>
    <div className="page-header"><div><p className="eyebrow">税金明细表</p><h1 className="title">{project.name}</h1><p className="subtitle">自动汇总增值税、附加税、土地增值税和企业所得税。当前只统计当前版本启用业态收入与启用业态成本，停用业态自动排除。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/land-vat`} className="btn btn-primary">土地增值税</Link><Link href={`/projects/${project.id}/product-maintenance`} className="btn">业态维护</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
