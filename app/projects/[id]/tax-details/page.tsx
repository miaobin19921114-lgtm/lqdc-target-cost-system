...
export default async function TaxDetailsPage({ params }: { params: { id: string } }) {
  ...
  return <main className="page"><ProjectTopNav projectId={project.id} projectName={project.name} current="税费测算总表" /><div className="container" style={{ maxWidth: 1400 }}>
    <div className="page-header"><div><p className="eyebrow">税金测算</p><h1 className="title">{project.name}</h1><p className="subtitle">金额单位统一为万元；土地增值税读取土增税清算分摊规则，所得税成本对象拆分读取所得税成本对象规则。</p></div><div className="actions" style={{ marginTop: 0 }}><button className="btn" disabled={locked} title={locked ? '当前版本已锁定，仅支持查看。' : undefined}>刷新税金测算</button><button className="btn" disabled={locked} title={locked ? '当前版本已锁定，仅支持查看。' : undefined}>同步至目标成本汇总表</button><Link href={`/projects/${project.id}/land-vat`} className="btn btn-primary">查看土地增值税明细</Link><Link href={`/projects/${project.id}/summary`} className="btn">查看目标成本汇总表</Link></div></div>
    <VersionContextBar projectName={project.name} versionName={version?.name} versionStatus={version?.status} editable={!locked} extra={[['启用业态', activeProducts.length], ['有效成本行', effective.effective.length]]} />
    {disabledProducts || effective.ignoredDisabled || effective.ignoredNonLeaf ? <StatusNotice title="已按有效口径过滤" tone="warning">已排除停用业态 {disabledProducts} 个、停用业态成本行 {effective.ignoredDisabled} 行、非末级历史成本行 {effective.ignoredNonLeaf} 行。</StatusNotice> : null}
    <div className="summary-strip"><div className="stat"><div className="stat-label">销项税额</div><div className="stat-value">{fmt(revenue.outputVat)}</div></div><div className="stat"><div className="stat-label">进项税额</div><div className="stat-value">{fmt(cost.inputVat)}</div></div><div className="stat"><div className="stat-label">应缴增值税</div><div className="stat-value">{fmt(quickTax.payableVat)}</div></div><div className="stat"><div className="stat-label">附加税费</div><div className="stat-value">{fmt(quickTax.surcharge)}</div></div><div className="stat"><div className="stat-label">土地增值税</div><div className="stat-value">{fmt(formalLandVat)}</div></div><div className="stat"><div className="stat-label">所得税</div><div className="stat-value">{fmt(formalIncomeTax)}</div></div></div>
    <section className="card" style={{ marginBottom: 18 }}><h2>按所得税成本对象拆分</h2><p className="meta">本表用于管理视角查看业态税费贡献；项目整体所得税仍按项目法人口径汇总，正式土增税以土地增值税清算测算表为准。</p>{objectRows.length ? <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 1450, borderCollapse: 'collapse', fontSize: 12 }}><thead><tr>{['业态', '成本归属组', '清算对象', '销项税', '进项税', '应缴增值税', '附加税', '所得税应税所得', '所得税', '税费合计(不含土增税)', '税后净利', '净利率'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 9, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{head}</th>)}</tr></thead><tbody>{objectRows.map((row) => <tr key={row.product.id}><td style={{ ...textCell, fontWeight: 800 }}>{row.product.name}</td><td style={textCell}>{row.costGroup}</td><td style={textCell}>{row.liquidationObject}</td><td style={moneyCell}>{fmt(row.outputVat)}</td><td style={moneyCell}>{fmt(row.inputVat)}</td><td style={{ ...moneyCell, fontWeight: 800 }}>{fmt(row.payableVat)}</td><td style={moneyCell}>{fmt(row.surcharge)}</td><td style={moneyCell}>{fmt(row.taxableIncome)}</td><td style={moneyCell}>{fmt(row.incomeTax)}</td><td style={{ ...moneyCell, fontWeight: 800 }}>{fmt(row.totalTax)}</td><td style={{ ...moneyCell, fontWeight: 900, color: row.netProfit >= 0 ? '#2f9e44' : '#e03131' }}>{fmt(row.netProfit)}</td><td style={textCell}>{pct(row.revenueInclusive ? row.netProfit / row.revenueInclusive : 0)}</td></tr>)}</tbody></table></div> : <EmptyState title="暂无可拆分的税费对象">请先维护启用业态、收入和有效成本明细；形成数据后本区会按所得税成本对象展示税费贡献。</EmptyState>}</section>
