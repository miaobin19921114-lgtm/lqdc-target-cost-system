...
export default async function TaxDetailsPage({ params }: { params: { id: string } }) {
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1400 }}>
    <div className="page-header"><div><p className="eyebrow">税金明细表</p><h1 className="title">{project.name}</h1><p className="subtitle">金额单位统一为万元；税率和利润率为百分比。本页会自动重算成本行，保证成本税额按万元口径参与税金测算。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/land-vat`} className="btn btn-primary">土地增值税</Link><Link href={`/projects/${project.id}/profit-analysis`} className="btn">业态利润</Link><Link href={`/projects/${project.id}/cost-allocation`} className="btn">成本分摊</Link><Link href={`/projects/${project.id}/summary`} className="btn">目标成本汇总</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
    ...
    <div className="summary-strip"><div className="stat"><div className="stat-label">含税总收入（万元）</div><div className="stat-value">{fmt(revenue.taxInclusive)}</div></div><div className="stat"><div className="stat-label">应缴增值税（万元）</div><div className="stat-value">{fmt(tax.payableVat)}</div></div><div className="stat"><div className="stat-label">土增税（万元）</div><div className="stat-value">{fmt(tax.landVat.landVat)}</div></div><div className="stat"><div className="stat-label">税费合计（万元）</div><div className="stat-value">{fmt(tax.totalTax)}</div></div></div>
    <section className="card" style={{ marginBottom: 18 }}><h2>按业态成本对象税金拆分</h2><p className="meta">本表按成本归属组拆分税金，金额单位均为万元。</p><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 1560, borderCollapse: 'collapse', fontSize: 12 }}><thead><tr>{['业态', '成本归属组', '成本对象', '销项税(万元)', '进项税(万元)', '应缴增值税(万元)', '附加税(万元)', '土增税(万元)', '所得税应税所得(万元)', '所得税(万元)', '税费合计(万元)', '税后净利(万元)', '净利率'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 9, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{head}</th>)}</tr></thead><tbody>{objectRows.length ? objectRows.map((row) => <tr key={row.product.id}><td style={{ ...textCell, fontWeight: 800 }}>{row.product.name}</td><td style={textCell}>{row.costGroup}</td><td style={textCell}>{taxObjectName(row.product)}</td><td style={moneyCell}>{fmt(row.outputVat)}</td><td style={moneyCell}>{fmt(row.inputVat)}</td><td style={{ ...moneyCell, fontWeight: 800 }}>{fmt(row.payableVat)}</td><td style={moneyCell}>{fmt(row.surcharge)}</td><td style={moneyCell}>{fmt(row.landVat)}</td><td style={moneyCell}>{fmt(row.taxableIncome)}</td><td style={moneyCell}>{fmt(row.incomeTax)}</td><td style={{ ...moneyCell, fontWeight: 800 }}>{fmt(row.totalTax)}</td><td style={{ ...moneyCell, fontWeight: 900, color: row.netProfit >= 0 ? '#2f9e44' : '#e03131' }}>{fmt(row.netProfit)}</td><td style={textCell}>{pct(row.revenueInclusive ? row.netProfit / row.revenueInclusive : 0)}</td></tr>) : <tr><td colSpan={13} style={{ padding: 12, color: 'var(--muted)' }}>暂无启用业态或可分摊成本。</td></tr>}</tbody></table></div></section>
    <section className="card"><h2>项目整体税金明细</h2><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 900, borderCollapse: 'collapse' }}><thead><tr>{['税费项目', '金额(万元)', '说明'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 10, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{head}</th>)}</tr></thead><tbody>{rows.map(([name, value, remark]) => <tr key={name}><td style={{ padding: 10, borderBottom: '1px solid var(--border)', fontWeight: 800 }}>{name}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)', textAlign: 'right', fontWeight: 800 }}>{fmt(value)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{remark}</td></tr>)}</tbody></table></div></section>
  </div></main>;
}
