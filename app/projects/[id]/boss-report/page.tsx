...
export default async function BossReportPage({ params }: { params: { id: string } }) {
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1280 }}>
    <div className="page-header no-print"><div><p className="eyebrow">老板汇报版</p><h1 className="title">{project.name}</h1><p className="subtitle">一页看结论：收入、成本、利润、税费、风险和下一步动作。口径已统一为商业专项收入 + 有效末级成本。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/report`} className="btn">经营报告</Link><Link href={`/projects/${project.id}/report-export`} className="btn">PDF/Word导出</Link><Link href={`/projects/${project.id}`} className="btn btn-primary">返回项目测算中心</Link></div></div>
