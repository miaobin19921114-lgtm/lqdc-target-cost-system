...
export default async function DashboardLite({ params }: { params: { id: string } }) {
  ...
  const decision = tax.netProfit < 0
    ? { level: '暂缓推进', color: '#e03131', reason: '基准方案已经亏损，应先复核售价、成本和税费口径。', action: '先暂停推进，优先做成本压降、售价复核和税费口径复核。' }
    : lossCells === 0 && netMargin >= 0.08 && priceDown5.netProfit >= 0 && costUp5.netProfit >= 0
      ? { level: '建议推进', color: '#2f9e44', reason: '基准净利率较好，主要压力情景下仍保持盈利。', action: '可进入深化方案、融资测算和关键成本锁定。' }
      : lossCells <= 4 && netMargin > 0
        ? { level: '谨慎推进', color: '#f08c00', reason: '基准方案盈利，但部分压力组合会明显压缩利润。', action: '先锁定土地、建安、售价和去化假设，再进入投决会。' }
        : { level: '暂缓推进', color: '#e03131', reason: '矩阵中亏损格子较多，项目抗压能力偏弱。', action: '建议先做方案优化和成本重构，再复测经营指标。' };
  ...
  const costStructure = [
    ['土地费', cost.landCost, '含税口径'],
    ['开发成本', cost.devCost, '不含税口径'],
    ['销管财费用', cost.saleManageFinance, '不含税口径'],
    ['进项税额', cost.inputVat, '税额口径']
  ] as const;
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1380 }}>
    <div className="page-header"><div><p className="eyebrow">经营看板</p><h1 className="title">经营总控</h1><p className="subtitle">定位为项目经营驾驶舱：看结论、看指标、看结构、看风险、看下一步动作。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/decision`} className="btn btn-primary">投决评审</Link><Link href={`/projects/${project.id}/report-print`} className="btn">打印版报告</Link><Link href={`/projects/${project.id}/sensitivity-report`} className="btn">敏感性报告</Link><Link href={`/projects/${project.id}/indicator-check`} className="btn">指标校验</Link><Link href={`/projects/${project.id}`} className="btn">项目测算中心</Link></div></div>
    ...
    <section className="card" style={{ marginBottom: 18 }}><h2>三、收入 / 成本 / 利润总览</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}><div><span className="meta">含税销售收入</span><div style={{ fontSize: 24, fontWeight: 900 }}>{fmt(revenue.taxInclusive)}</div></div><div><span className="meta">含税目标成本</span><div style={{ fontSize: 24, fontWeight: 900 }}>{fmt(cost.taxInclusive)}</div></div><div><span className="meta">税前利润</span><div style={{ fontSize: 24, fontWeight: 900, color: statusColor(tax.profitBeforeIncomeTax) }}>{fmt(tax.profitBeforeIncomeTax)}</div></div><div><span className="meta">税后净利</span><div style={{ fontSize: 24, fontWeight: 900, color: statusColor(tax.netProfit) }}>{fmt(tax.netProfit)}</div></div></div></section>

    <section className="card" style={{ marginBottom: 18 }}><h2>四、成本结构分析</h2><div style={{ display: 'grid', gap: 12 }}>{costStructure.map(([name, amount, note]) => <div key={name}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}><b>{name}</b><span>{fmt(amount)} · {note}</span></div><div style={{ height: 10, background: '#eef2f6', borderRadius: 999, marginTop: 6 }}><div style={{ width: barWidth(Number(amount), cost.taxInclusive || 1), height: 10, borderRadius: 999, background: '#0b7285' }} /></div></div>)}</div></section>

    <section className="card" style={{ marginBottom: 18 }}><h2>五、税费与利润影响</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}><div><span className="meta">应缴增值税</span><div style={{ fontSize: 22, fontWeight: 900 }}>{fmt(tax.payableVat)}</div></div><div><span className="meta">附加税</span><div style={{ fontSize: 22, fontWeight: 900 }}>{fmt(tax.surcharge)}</div></div><div><span className="meta">土地增值税</span><div style={{ fontSize: 22, fontWeight: 900 }}>{fmt(tax.landVat.landVat)}</div></div><div><span className="meta">企业所得税</span><div style={{ fontSize: 22, fontWeight: 900 }}>{fmt(tax.incomeTax)}</div></div><div><span className="meta">综合税负</span><div style={{ fontSize: 22, fontWeight: 900 }}>{revenue.taxInclusive ? pct(tax.totalTax / revenue.taxInclusive) : '0%'}</div></div></div></section>

    <section className="card" style={{ marginBottom: 18 }}><h2>六、敏感性预警</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}><div><span className="meta">售价下跌5%</span><div style={{ fontSize: 22, fontWeight: 900, color: statusColor(priceDown5.netProfit) }}>{fmt(priceDown5.netProfit)}</div></div><div><span className="meta">成本上涨5%</span><div style={{ fontSize: 22, fontWeight: 900, color: statusColor(costUp5.netProfit) }}>{fmt(costUp5.netProfit)}</div></div><div><span className="meta">矩阵亏损格子</span><div style={{ fontSize: 22, fontWeight: 900, color: lossCells ? '#f08c00' : '#2f9e44' }}>{lossCells}/{matrixCells.length}</div></div></div></section>

    <section className="card" style={{ marginBottom: 18 }}><h2>七、数据完整性检查</h2><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 920, borderCollapse: 'collapse' }}><thead><tr>{['检查项', '状态', '说明', '入口'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 10, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{head}</th>)}</tr></thead><tbody>{checks.map((row) => <tr key={row.name}><td style={{ padding: 10, borderBottom: '1px solid var(--border)', fontWeight: 800 }}>{row.name}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)', color: row.ok ? '#2f9e44' : '#f08c00', fontWeight: 900 }}>{row.ok ? '正常' : '需关注'}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{row.text}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}><Link className="btn" href={`/projects/${project.id}/${row.href}`}>进入</Link></td></tr>)}</tbody></table></div></section>

    <section className="card" style={{ marginBottom: 18 }}><h2>八、下一步动作入口</h2>{issueChecks.length ? <p className="meta">当前优先处理 {issueChecks.length} 个关注项：{issueChecks.map((item) => item.name).join('、')}。</p> : <p className="meta">当前关键检查项正常，可进入投决报告或打印经营报告。</p>}<div className="actions"><Link href={`/projects/${project.id}/indicator-check`} className="btn">指标校验中心</Link><Link href={`/projects/${project.id}/revenue`} className="btn">收入测算</Link><Link href={`/projects/${project.id}/costs-batch`} className="btn">目标成本</Link><Link href={`/projects/${project.id}/tax-report`} className="btn">税务报告</Link><Link href={`/projects/${project.id}/decision`} className="btn btn-primary">投决评审</Link></div></section>
