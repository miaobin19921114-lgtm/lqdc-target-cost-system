...
export default async function DashboardLite({ params }: { params: { id: string } }) {
  ...
  const checks = [
    { name: '投决评级', ok: decision.level !== '暂缓推进', text: `${decision.level}：${decision.reason}`, href: 'report' },
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1380 }}>
    <div className="page-header"><div><p className="eyebrow">经营总控看板</p><h1 className="title">{project.name}</h1><p className="subtitle">统一读取当前启用版本，按目标成本汇总、税金明细、成本分摊同一口径展示项目是否赚钱。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/report`} className="btn btn-primary">经营报告</Link><Link href={`/projects/${project.id}/sensitivity`} className="btn">敏感性</Link><Link href={`/projects/${project.id}/profit-analysis`} className="btn">业态利润</Link><Link href={`/projects/${project.id}/summary`} className="btn">目标成本汇总</Link><Link href={`/projects/${project.id}/summary-check`} className="btn">汇总校验</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
    <section className="card" style={{ marginBottom: 18, borderColor: decision.color }}><div className="meta">投资决策评级</div><div style={{ fontSize: 30, fontWeight: 900, color: decision.color, marginTop: 6 }}>{decision.level}</div><p className="meta" style={{ marginTop: 8 }}>{decision.reason}</p><div className="actions"><Link href={`/projects/${project.id}/report`} className="btn btn-primary">查看经营报告</Link><Link href={`/projects/${project.id}/sensitivity`} className="btn">查看敏感性</Link></div></section>
    ...
    <section className="card"><h2>最近导入批次</h2>{activeBatches.length ? <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 860, borderCollapse: 'collapse' }}><thead><tr>{['文件', '模式', '行数', '含税合计', '状态'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 10, borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{head}</th>)}</tr></thead><tbody>{activeBatches.map((batch) => <tr key={batch.id}><td style={{ padding: 10, borderBottom: '1px solid var(--border)', fontWeight: 800 }}><Link href={`/projects/${project.id}/import-batches/${batch.id}`}>{batch.fileName}</Link></td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{batch.importMode}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{batch.rowCount}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{fmt(batch.taxInclusiveTotal)}</td><td style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{batch.status}</td></tr>)}</tbody></table></div> : <p className="meta">暂无导入批次。</p>}<div className="actions"><Link href={`/projects/${project.id}/sensitivity`} className="btn btn-primary">敏感性测算</Link><Link href={`/projects/${project.id}/report`} className="btn">经营报告</Link><Link href={`/projects/${project.id}/export`} className="btn">Excel导入导出</Link><Link href={`/projects/${project.id}/import-batches`} className="btn">导入批次</Link><Link href={`/projects/${project.id}/cost-mapping`} className="btn">科目映射</Link></div></section>
  </div></main>;
}
