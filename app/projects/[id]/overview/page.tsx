...
export default async function ProjectOverviewPage({ params, searchParams }: { params: { id: string }, searchParams?: { saved?: string; productSaved?: string; preset?: string; rows?: string } }) {
  ...
  return (
    <main className="page" style={{ background: '#eef3f8' }}><div className="container" style={{ maxWidth: 1500 }}>
      <div className="page-header" style={{ alignItems: 'flex-start' }}><div><p className="eyebrow">项目概况表</p><h1 className="title">{project.name}</h1><p className="subtitle">概况表只显示启用业态；已停用业态统一到“业态维护”页面查看、恢复或删除。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/revenue`} className="btn btn-primary">收入明细表</Link><Link href={`/projects/${project.id}/land`} className="btn">土地费用明细</Link><Link href={`/projects/${project.id}/product-maintenance`} className="btn">业态维护</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <Block title="二、主要经济技术指标 - 2.1 业态/产品构成" note="本项目只显示启用业态；销售单价不在这里维护，进入收入明细表维护。">
