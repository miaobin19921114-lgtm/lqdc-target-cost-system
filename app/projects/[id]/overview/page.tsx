...
function Block({ title, note, children }: { title: string; note: string; children: ReactNode }) { return <section style={{ border: '1px solid #d9e2ec', borderRadius: 12, background: '#fff', overflow: 'hidden' }}><div style={{ padding: '12px 14px', background: '#f8fafc', borderBottom: '1px solid #d9e2ec' }}><h2 style={{ margin: 0, fontSize: 17 }}>{title}</h2><p className="meta" style={{ margin: '5px 0 0' }}>{note}</p></div><div style={{ padding: 14 }}>{children}</div></section>; }
function MetricCard({ label, value, unit, note }: { label: string; value: unknown; unit?: string; note?: string }) { return <div className="stat" style={{ minHeight: 86 }}><div className="stat-label">{label}</div><div className="stat-value">{displayValue(value)}{unit || ''}</div>{note ? <div className="meta">{note}</div> : null}</div>; }
...
export default async function ProjectOverviewPage({ params, searchParams }: { params: { id: string }, searchParams?: { saved?: string; productSaved?: string; rows?: string } }) {
  ...
  return <main className="page" style={{ background: '#eef3f8' }}><div className="container" style={{ maxWidth: 1500 }}>
    <div className="page-header" style={{ alignItems: 'flex-start' }}><div><p className="eyebrow">项目基础</p><h1 className="title">项目概况</h1><p className="subtitle">项目概况是目标成本测算指标入口，优先维护土地、规划、面积、业态、车位、充电桩、景观、周界、出入口、楼栋、单元和电梯等关键口径。</p></div><div className="actions" style={{ marginTop: 0 }}><button form="overview-form" className="btn btn-primary">保存项目概况</button><Link href={`/projects/${project.id}/product-maintenance`} className="btn">业态维护</Link><Link href={`/projects/${project.id}/costs-batch`} className="btn">目标成本测算</Link><Link href={`/projects/${project.id}`} className="btn">返回项目测算中心</Link></div></div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 14 }}>
      ...
      <Block title="九、项目配置参数" note="控制装配式、精装交付、商业/地库/配套/示范区装修和采暖口径。装配式和采暖从当前启用业态中多选适用范围，不再手写。"><div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}><div><b>9.1 装配式配置</b><ConfigGrid>{selectFor(project, 'isPrefabricated', '是否装配式', yesNo, 'false')}<ProductScopeSelect name="prefabricatedScope" label="装配式适用范围" products={scopeProductNames} value={project.prefabricatedScope} note="从当前启用业态中多选；不选则不启用装配式科目。" />{inputFor(project, ['prefabricationRate', '装配率%', 'number'])}<label style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#475467' }}>装配式结构形式<input form="overview-form" name="prefabricatedSystem" defaultValue={valueOf(project, 'prefabricatedSystem')} placeholder="PC构件/叠合板/预制楼梯/预制墙板" style={formField} /></label></ConfigGrid></div><div><b>9.2 住宅精装配置</b><ConfigGrid>{selectFor(project, 'residentialPublicFitoutStandard', '住宅公区装修标准', fitoutStandards, '标准', '住宅公区默认装修，不建议关闭。')}{selectFor(project, 'undergroundLobbyFitoutStandard', '地下归家装修标准', [['无地下归家', '无地下归家'], ['标准', '标准'], ['中档', '中档'], ['高档', '高档']], '标准')}{selectFor(project, 'residentialFitoutDelivery', '是否精装交付', yesNo, 'false')}{selectFor(project, 'residentialFitoutType', '户内精装修类型', [['硬装', '硬装'], ['软装', '软装'], ['硬装+软装', '硬装+软装']], '硬装')}{selectFor(project, 'residentialFitoutStandard', '户内交付标准', [['毛坯', '毛坯'], ['简装', '简装'], ['中装', '中装'], ['精装', '精装'], ['豪装', '豪装']], '毛坯')}<div style={{ color: '#667085', fontSize: 13, lineHeight: 1.6 }}>精装修目前按住宅精装、商业公区、地库品质、配套精装等开关控制；若要做“按启用业态多选精装范围”，需要下一步新增精装适用范围字段。</div></ConfigGrid></div><div><b>9.3 商业、地库、配套及示范区装修</b><ConfigGrid>{selectFor(project, 'commercialPublicFitout', '是否商业公区装修', yesNo, 'false')}{selectFor(project, 'commercialPublicFitoutStandard', '商业公区装修标准', fitoutStandards, '标准')}{selectFor(project, 'shopDeliveryStandard', '商铺交付标准', [['毛坯', '毛坯'], ['简装', '简装'], ['中装', '中装'], ['精装', '精装']], '毛坯')}{selectFor(project, 'basementQualityUpgrade', '是否地库品质提升', yesNo, 'false')}{selectFor(project, 'basementQualityStandard', '地库品质标准', [['基础美化', '基础美化'], ['中档', '中档'], ['高档', '高档']], '基础美化')}{selectFor(project, 'propertyFitout', '物业用房精装', yesNo, 'false')}{selectFor(project, 'communityFitout', '社区用房精装', yesNo, 'false')}{selectFor(project, 'supportFitout', '配套用房精装', yesNo, 'false')}{selectFor(project, 'hasSalesOffice', '是否有售楼部', yesNo, 'false')}{selectFor(project, 'salesOfficeFitoutType', '售楼部装修类型', [['硬装', '硬装'], ['软装', '软装'], ['硬装+软装', '硬装+软装']], '硬装+软装')}{selectFor(project, 'hasShowFlat', '是否有样板间', yesNo, 'false')}{selectFor(project, 'showFlatFitoutType', '样板间装修类型', [['硬装', '硬装'], ['软装', '软装'], ['硬装+软装', '硬装+软装'], ['全部', '全部']], '全部')}</ConfigGrid></div><div><b>9.4 采暖配置</b><ConfigGrid>{selectFor(project, 'heatingEnabled', '是否采暖', yesNo, 'false')}<ProductScopeSelect name="heatingScope" label="采暖适用范围" products={scopeProductNames} value={project.heatingScope} note="从当前启用业态中多选；不选则不启用采暖科目。" /><label style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#475467' }}>采暖形式<input form="overview-form" name="heatingType" defaultValue={valueOf(project, 'heatingType')} placeholder="地暖/散热器/集中供暖/空气源" style={formField} /></label></ConfigGrid></div></div></Block>
