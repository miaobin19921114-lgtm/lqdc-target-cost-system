...
function productInput(name: string, label: string, type = 'number') {
  return <label style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#475467' }}>{label}<input name={name} type={type} step={type === 'number' ? '0.01' : undefined} style={{ height: 34, border: '1px solid #d9e2ec', borderRadius: 6, padding: '4px 8px' }} /></label>;
}
    ...
    <main className="page" style={{ background: '#eef3f8' }}>
      <div className="container" style={{ maxWidth: 1500 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <Block title="二、主要经济技术指标 - 2.1 业态/产品构成" note="业态/产品构成已并入概况表，是收入测算、成本分摊、可售面积校验的源头。">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 14 }}>
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', marginBottom: 10 }}><div><b>业态汇总校验</b><p className="meta" style={{ margin: '4px 0 0' }}>可售业态 {saleableProducts.length} 个；不可售/配套/地下空间 {nonSaleableProducts.length} 个。</p></div><form action={`/api/projects/${project.id}/products/presets`} method="post"><button className="btn btn-primary">生成模板业态</button></form></div>
                ...
                <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1100, fontSize: 12 }}><thead><tr>{['分类', '业态', '建筑面积', '计容面积', '可售面积', '不可售面积', '销售单价', '货值', '销售', '分摊'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 8, borderBottom: '1px solid #d9e2ec', color: '#667085' }}>{head}</th>)}</tr></thead><tbody>{products.length === 0 ? <tr><td colSpan={10} style={{ padding: 14, color: '#667085' }}>暂无业态。可先点击“生成模板业态”，再录入面积和售价。</td></tr> : products.map((item) => { const revenue = Number(item.saleableArea || 0) * Number(item.salePrice || 0); return <tr key={item.id}><td style={{ padding: 8, borderBottom: '1px solid #eef2f6' }}>{getCategory(item.name, item.remark)}</td><td style={{ padding: 8, borderBottom: '1px solid #eef2f6', fontWeight: 800 }}>{item.name}</td><td style={{ padding: 8, borderBottom: '1px solid #eef2f6' }}>{fmt(item.buildingArea)}㎡</td><td style={{ padding: 8, borderBottom: '1px solid #eef2f6' }}>{fmt(item.capacityArea)}㎡</td><td style={{ padding: 8, borderBottom: '1px solid #eef2f6' }}>{fmt(item.saleableArea)}㎡</td><td style={{ padding: 8, borderBottom: '1px solid #eef2f6' }}>{fmt(item.nonSaleableArea)}㎡</td><td style={{ padding: 8, borderBottom: '1px solid #eef2f6' }}>{fmt(item.salePrice)}</td><td style={{ padding: 8, borderBottom: '1px solid #eef2f6' }}>{fmt(revenue)}</td><td style={{ padding: 8, borderBottom: '1px solid #eef2f6' }}>{item.isSaleable ? '参与' : '不参与'}</td><td style={{ padding: 8, borderBottom: '1px solid #eef2f6' }}>{item.participateAllocation ? '参与' : '不参与'}</td></tr>; })}</tbody></table></div>
              </div>
              <form action={`/api/projects/${project.id}/products`} method="post" style={{ border: '1px solid #eef2f6', borderRadius: 10, padding: 12, background: '#fcfdff', display: 'flex', flexDirection: 'column', gap: 10 }}><input type="hidden" name="returnPath" value="overview" /><b>新增 / 更新业态</b><label style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#475467' }}>业态名称<select name="name" required defaultValue="" style={{ height: 34, border: '1px solid #d9e2ec', borderRadius: 6, padding: '4px 8px' }}><option value="" disabled>请选择固定业态</option>{presetGroups.map((group) => <optgroup key={group.category} label={group.category}>{group.names.map((name) => <option key={name} value={name}>{name}</option>)}</optgroup>)}</select></label>{productInput('buildingArea', '建筑面积㎡')}{productInput('capacityArea', '计容面积㎡')}{productInput('saleableArea', '可售面积㎡')}{productInput('nonSaleableArea', '不可售面积㎡')}{productInput('salePrice', '销售单价 元/㎡')}{productInput('allocationWeight', '分摊权重')}<label style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#475467' }}>备注<input name="remark" placeholder="面积口径、售价依据" style={{ height: 34, border: '1px solid #d9e2ec', borderRadius: 6, padding: '4px 8px' }} /></label><label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#475467' }}><input name="isSaleable" type="checkbox" defaultChecked style={{ width: 'auto' }} />参与销售</label><label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#475467' }}><input name="participateAllocation" type="checkbox" defaultChecked style={{ width: 'auto' }} />参与成本分摊</label><button className="btn btn-primary">保存业态</button></form>
            </div>
          </Block>
