...
const basicFields = [
  ...
  ['landArea', '土地面积㎡', 'number'], ['landAreaMu', '土地面积亩', 'number'], ['redLineArea', '用地红线面积㎡', 'number'],
...
function MetricCard({ label, value, unit, note }: { label: string; value: unknown; unit?: string; note?: string }) {
  return <div className="stat" style={{ minHeight: 86 }}><div className="stat-label">{label}</div><div className="stat-value">{fmt(value)}{unit || ''}</div>{note ? <div className="meta">{note}</div> : null}</div>;
}
...
export default async function ProjectOverviewPage({ params, searchParams }: { params: { id: string }, searchParams?: { saved?: string; productSaved?: string; rows?: string } }) {
  ...
  return <main className="page" style={{ background: '#eef3f8' }}><div className="container" style={{ maxWidth: 1500 }}>
    <div className="page-header" style={{ alignItems: 'flex-start' }}><div><p className="eyebrow">项目基础</p><h1 className="title">项目概况</h1><p className="subtitle">项目概况是目标成本测算指标入口，优先维护土地、规划、面积、业态、车位、充电桩、景观、周界、出入口、楼栋、单元和电梯等关键口径。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/indicator-check`} className="btn btn-primary">指标校验</Link><Link href={`/projects/${project.id}`} className="btn">项目测算中心</Link></div></div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 14 }}>
      ...
      <Block title="五、场地、景观、道路与围墙指标" note="用于围墙、出入口、临设、三通一平、道路总平、景观、室外管网等成本测算。"><FieldGrid project={project} fields={siteEngineeringFields} /></Block>
      <Block title="六、地下室 / 公区 / 配套面积" note="用于地下室、主楼地下室、人防、公区、大堂、售楼部、样板间、物业/社区用房等成本测算。"><FieldGrid project={project} fields={basementPublicFields} /></Block>
      ...
      <Block title="九、项目配置参数" note="控制装配式、精装交付、商业/地库/配套/示范区装修和采暖口径。"><div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}><div><b>9.1 装配式配置</b><ConfigGrid>{selectFor(project, 'isPrefabricated', '是否装配式', yesNo, 'false')}{textFor(project, 'prefabricatedScope', '装配式适用范围', '高层/洋房/商业/配套/地下室/全部')}{inputFor(project, ['prefabricationRate', '装配率%', 'number'])}{textFor(project, 'prefabricatedSystem', '装配式结构形式', 'PC构件/叠合板/预制楼梯/预制墙板')}</ConfigGrid></div><div><b>9.2 住宅精装配置</b><ConfigGrid>{selectFor(project, 'residentialPublicFitoutStandard', '住宅公区装修标准', fitoutStandards, '标准', '住宅公区默认装修，不建议关闭。')}{selectFor(project, 'undergroundLobbyFitoutStandard', '地下归家装修标准', [['无地下归家', '无地下归家'], ['标准', '标准'], ['中档', '中档'], ['高档', '高档']], '标准')}{selectFor(project, 'residentialFitoutDelivery', '是否精装交付', yesNo, 'false')}{selectFor(project, 'residentialFitoutType', '户内精装修类型', [['硬装', '硬装'], ['软装', '软装'], ['硬装+软装', '硬装+软装']], '硬装')}{selectFor(project, 'residentialFitoutStandard', '户内交付标准', [['毛坯', '毛坯'], ['简装', '简装'], ['中装', '中装'], ['精装', '精装'], ['豪装', '豪装']], '毛坯')}</ConfigGrid></div><div><b>9.3 商业、地库、配套及示范区装修</b><ConfigGrid>{selectFor(project, 'commercialPublicFitout', '是否商业公区装修', yesNo, 'false')}{selectFor(project, 'commercialPublicFitoutStandard', '商业公区装修标准', [['标准', '标准'], ['中档', '中档'], ['高档', '高档']], '标准')}{selectFor(project, 'shopDeliveryStandard', '商铺交付标准', [['毛坯', '毛坯'], ['简装', '简装'], ['统一装修', '统一装修']], '毛坯')}{selectFor(project, 'basementQualityUpgrade', '是否地库品质提升', yesNo, 'false')}{selectFor(project, 'basementQualityStandard', '地库品质提升标准', [['基础美化', '基础美化'], ['中档包装', '中档包装'], ['高档归家', '高档归家']], '基础美化')}{selectFor(project, 'propertyFitout', '是否物业用房装修', yesNo, 'true')}{selectFor(project, 'communityFitout', '是否社区用房装修', yesNo, 'true')}{selectFor(project, 'hasSalesOffice', '是否有售楼部', yesNo, 'true')}{selectFor(project, 'hasShowFlat', '是否有样板间', yesNo, 'true')}</ConfigGrid></div><div><b>9.4 采暖配置</b><ConfigGrid>{selectFor(project, 'heatingEnabled', '是否配置采暖', yesNo, 'false')}{textFor(project, 'heatingScope', '采暖适用范围', '高层/洋房/商业/地库局部/配套/全部')}{selectFor(project, 'heatingType', '采暖形式', [['', '未配置'], ['地暖', '地暖'], ['散热器', '散热器'], ['集中供暖换热站', '集中供暖换热站'], ['空调采暖', '空调采暖'], ['其他', '其他']], '')}</ConfigGrid></div></div></Block>
      <Block title="十、备注与口径说明" note="保留项目定位、面积口径、取数来源、特殊假设等，不参与计算。"><textarea form="overview-form" name="remark" defaultValue={project.remark || ''} placeholder="填写项目定位、测算边界、面积口径、取数说明等" style={{ width: '100%', minHeight: 90, border: '1px solid #d9e2ec', borderRadius: 8, padding: 10 }} /></Block>
