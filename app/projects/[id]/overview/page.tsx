...
const basicFields = [
  ...
  ['landAreaMu', '土地面积亩', 'number'],
  ['redLineArea', '用地红线面积㎡', 'number'],
...
const buildingIndicatorFields = [
  ...
  ['householdCount', '户数/套数', 'number'],
  ['elevatorCount', '电梯数量', 'number'],
  ['aboveGroundFloors', '典型地上层数', 'number'],
  ['basementFloors', '地下层数', 'number'],
  ['standardFloorArea', '标准层面积㎡', 'number'],
  ['standardFloorHeight', '标准层层高m', 'number'],
  ['basementFloorHeight', '地下室层高m', 'number']
] as const;
...
const siteEngineeringFields = [
  ...
  ['formalGateCount', '正式出入口数量', 'number'],
  ['temporaryGateCount', '临时出入口数量', 'number'],
  ['temporaryFacilityArea', '临设面积㎡', 'number'],
  ['siteLevelingArea', '场平面积㎡', 'number'],
  ['landscapeArea', '景观总面积㎡', 'number'],
  ['hardscapeArea', '硬景面积㎡', 'number'],
  ['softscapeArea', '软景面积㎡', 'number'],
  ['greenArea', '绿地面积㎡', 'number'],
  ['waterFeatureArea', '水景面积㎡', 'number'],
  ['childrenActivityArea', '儿童活动场地㎡', 'number'],
  ['elevatedFloorLandscapeArea', '架空层景观面积㎡', 'number'],
  ['roadArea', '道路总面积㎡', 'number'],
  ['fireRoadArea', '消防道路面积㎡', 'number'],
  ['asphaltRoadArea', '沥青路面面积㎡', 'number']
] as const;
...
const basementPublicFields = [
  ...
  ['salesOfficeArea', '售楼部面积㎡', 'number'],
  ['showFlatArea', '样板间面积㎡', 'number'],
  ['propertyManagementArea', '物业用房面积㎡', 'number'],
  ['communityServiceArea', '社区用房面积㎡', 'number']
] as const;

const civilEngineeringFields = [
  ['baseArea', '基底/占地面积㎡', 'number'],
  ['pileFoundationArea', '桩基面积㎡', 'number'],
  ['earthworkVolume', '土方量m³', 'number'],
  ['waterproofArea', '防水面积㎡', 'number'],
  ['roofArea', '屋面面积㎡', 'number'],
  ['insulationArea', '保温面积㎡', 'number'],
  ['facadeArea', '外墙面积㎡', 'number'],
  ['windowArea', '门窗面积㎡', 'number'],
  ['railingLength', '栏杆长度m', 'number']
] as const;

const mepEquipmentFields = [
  ['powerRoomCount', '配电房数量', 'number'],
  ['pumpRoomCount', '水泵房数量', 'number'],
  ['firePoolVolume', '消防水池容积m³', 'number']
] as const;
...
function inputFor(project: any, [name, label, type]: readonly [string, string, string], formId = 'overview-form') {
  return <label key={name} style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#475467' }}>
    {label}
    <input form={formId} name={name} type={type} step={type === 'number' ? '0.01' : undefined} defaultValue={valueOf(project, name)} style={formField} />
  </label>;
}
...
function selectFor(project: any, name: string, label: string, options: readonly (readonly [string, string])[], defaultValue?: string, note?: string) {
  ...
  return <label key={name} style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#475467' }}>
    {label}
    <select form="overview-form" name={name} defaultValue={value} style={formField}>
...
function textFor(project: any, name: string, label: string, placeholder?: string) {
  return <label key={name} style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#475467' }}>
    {label}
    <input form="overview-form" name={name} defaultValue={valueOf(project, name)} placeholder={placeholder} style={formField} />
  </label>;
}
...
function addInput(name: string, label: string) {
  return <label style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#475467' }}>
    {label}
    <input name={name} type="number" step="0.01" style={formField} />
  </label>;
}
    ...
    <div className="container" style={{ maxWidth: 1500 }}>
      <div className="page-header" style={{ alignItems: 'flex-start' }}>
        <div>
          ...
          <p className="subtitle">项目概况是全系统基础资料入口。项目配置参数控制装配式、精装交付、商业/地库/配套/示范区装修和采暖口径。</p>
        </div>
        <div className="actions" style={{ marginTop: 0 }}>
          <Link href={`/projects/${project.id}/indicator-check`} className="btn btn-primary">指标校验</Link>
      ...
      <div className="summary-strip" style={{ marginBottom: 14 }}>
        ...
        <div className="stat"><div className="stat-label">启用业态</div><div className="stat-value">{products.length}</div></div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <Block title="一、基础信息与规划指标" note="维护项目名称、城市板块、土地面积、容积率、总建面、计容建面、可售面积等全局口径。土地费金额不放这里，进入土地费用明细表。">
          <FieldGrid project={project} fields={basicFields} />
        </Block>

        <Block title="二、项目配置参数" note="控制装配式、精装交付、住宅公区、商业公区、地库品质提升、物业社区配套、示范区和采暖。住宅公区默认装修，只控制档次。">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <b>2.1 装配式配置</b>
              <ConfigGrid>
                {selectFor(project, 'isPrefabricated', '是否装配式', yesNo, 'false')}
                {textFor(project, 'prefabricatedScope', '装配式适用范围', '高层/洋房/商业/配套/地下室/全部')}
                {inputFor(project, ['prefabricationRate', '装配率%', 'number'])}
                {textFor(project, 'prefabricatedSystem', '装配式结构形式', 'PC构件/叠合板/预制楼梯/预制墙板')}
              </ConfigGrid>
            </div>
            <div>
              <b>2.2 住宅精装配置</b>
              <ConfigGrid>
                {selectFor(project, 'residentialPublicFitoutStandard', '住宅公区装修标准', fitoutStandards, '标准', '住宅公区默认装修，不建议关闭。')}
                {selectFor(project, 'undergroundLobbyFitoutStandard', '地下归家装修标准', [['无地下归家', '无地下归家'], ['标准', '标准'], ['中档', '中档'], ['高档', '高档']], '标准')}
                {selectFor(project, 'residentialFitoutDelivery', '是否精装交付', yesNo, 'false')}
                {selectFor(project, 'residentialFitoutType', '户内精装修类型', [['硬装', '硬装'], ['软装', '软装'], ['硬装+软装', '硬装+软装']], '硬装')}
                {selectFor(project, 'residentialFitoutStandard', '户内交付标准', [['毛坯', '毛坯'], ['简装', '简装'], ['中装', '中装'], ['精装', '精装'], ['豪装', '豪装']], '毛坯')}
              </ConfigGrid>
            </div>
            <div>
              <b>2.3 商业、地库、配套及示范区装修</b>
              <ConfigGrid>
                {selectFor(project, 'commercialPublicFitout', '是否商业公区装修', yesNo, 'false')}
                {selectFor(project, 'commercialPublicFitoutStandard', '商业公区装修标准', [['标准', '标准'], ['中档', '中档'], ['高档', '高档']], '标准')}
                {selectFor(project, 'shopDeliveryStandard', '商铺交付标准', [['毛坯', '毛坯'], ['简装', '简装'], ['统一装修', '统一装修']], '毛坯')}
                {selectFor(project, 'basementQualityUpgrade', '是否地库品质提升', yesNo, 'false')}
                {selectFor(project, 'basementQualityStandard', '地库品质提升标准', [['基础美化', '基础美化'], ['中档包装', '中档包装'], ['高档归家', '高档归家']], '基础美化')}
                {selectFor(project, 'propertyFitout', '是否物业用房装修', yesNo, 'true')}
                {selectFor(project, 'communityFitout', '是否社区用房装修', yesNo, 'true')}
                {selectFor(project, 'supportFitout', '是否配套用房装修', yesNo, 'false')}
                {selectFor(project, 'hasSalesOffice', '是否有售楼部', yesNo, 'true')}
                {selectFor(project, 'salesOfficeFitoutType', '售楼部装修类型', [['硬装', '硬装'], ['软装', '软装'], ['硬装+软装', '硬装+软装']], '硬装+软装')}
                {selectFor(project, 'hasShowFlat', '是否有样板间', yesNo, 'true')}
                {selectFor(project, 'showFlatFitoutType', '样板间装修类型', [['硬装', '硬装'], ['软装', '软装'], ['家具家电', '家具家电'], ['全部', '全部']], '全部')}
              </ConfigGrid>
            </div>
            <div>
              <b>2.4 采暖配置</b>
              <ConfigGrid>
                {selectFor(project, 'heatingEnabled', '是否配置采暖', yesNo, 'false')}
                {textFor(project, 'heatingScope', '采暖适用范围', '高层/洋房/商业/地库局部/配套/全部')}
                {selectFor(project, 'heatingType', '采暖形式', [['', '未配置'], ['地暖', '地暖'], ['散热器', '散热器'], ['集中供暖换热站', '集中供暖换热站'], ['空调采暖', '空调采暖'], ['其他', '其他']], '')}
              </ConfigGrid>
              <p className="meta" style={{ margin: '8px 0 0' }}>户内地暖构造触发条件：采暖=是 + 精装交付=是 + 采暖形式=地暖。</p>
            </div>
          </div>
        </Block>

        <Block title="三、业态 / 产品构成" note="维护启用业态的建面、计容面积、可售面积、不可售面积和分摊口径；销售单价进入收入测算页维护。">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 330px', gap: 14 }}>
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', marginBottom: 10 }}>
                <div><b>本项目启用业态</b><p className="meta" style={{ margin: '4px 0 0' }}>可售 {saleableProducts.length} 个；不可售/配套/地下空间 {nonSaleableProducts.length} 个。</p></div>
                <button form="overview-products-form" className="btn btn-primary">保存启用业态</button>
              </div>
              <div style={{ overflowX: 'auto', marginBottom: 12 }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 720, fontSize: 13 }}>
                  <thead><tr>{['口径', '概况表总控', '启用业态合计', '差异'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 9, borderBottom: '1px solid #d9e2ec', color: '#667085' }}>{head}</th>)}</tr></thead>
                  <tbody>{[['建筑面积', Number(project.totalBuildingArea || 0), productBuildingArea], ['计容面积', Number(project.capacityBuildingArea || 0), productCapacityArea], ['可售面积', Number(project.saleableArea || 0), productSaleableArea]].map(([name, overview, product]) => <tr key={String(name)}><td style={{ padding: 9, borderBottom: '1px solid #eef2f6', fontWeight: 800 }}>{name}</td><td style={{ padding: 9, borderBottom: '1px solid #eef2f6' }}>{fmt(overview)}㎡</td><td style={{ padding: 9, borderBottom: '1px solid #eef2f6' }}>{fmt(product)}㎡</td><td style={{ padding: 9, borderBottom: '1px solid #eef2f6', color: Math.abs(Number(overview) - Number(product)) > 1 ? '#e03131' : '#2f9e44', fontWeight: 900 }}>{fmt(Number(overview) - Number(product))}㎡</td></tr>)}</tbody>
                </table>
              </div>
              <form id="overview-products-form" action={`/api/projects/${project.id}/products/batch`} method="post" />
              <input form="overview-products-form" type="hidden" name="rowCount" value={products.length} />
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1120, fontSize: 12 }}>
                  <thead><tr>{['分类', '业态', '建筑面积', '计容面积', '可售面积', '不可售面积', '分摊权重', '销售', '分摊', '备注'].map((head) => <th key={head} style={{ ...cell, textAlign: 'left', color: '#667085' }}>{head}</th>)}</tr></thead>
                  <tbody>{products.length === 0 ? <tr><td colSpan={10} style={{ padding: 14, color: '#667085' }}>本项目暂无启用业态。请在右侧从后台模板添加，或到业态维护页恢复。</td></tr> : products.map((item, index) => <tr key={item.id} style={{ background: index % 2 ? '#fff' : '#fcfdff' }}><td style={cell}>{getCategory(item.name, item.remark)}</td><td style={{ ...cell, fontWeight: 800 }}><input form="overview-products-form" type="hidden" name={`productId-${index}`} value={item.id} /><input form="overview-products-form" type="hidden" name={`name-${index}`} value={item.name} />{item.name}</td><td style={{ ...cell, padding: 0 }}><input form="overview-products-form" name={`buildingArea-${index}`} type="number" step="0.01" defaultValue={Number(item.buildingArea || 0) || ''} style={tableInput} /></td><td style={{ ...cell, padding: 0 }}><input form="overview-products-form" name={`capacityArea-${index}`} type="number" step="0.01" defaultValue={Number(item.capacityArea || 0) || ''} style={tableInput} /></td><td style={{ ...cell, padding: 0 }}><input form="overview-products-form" name={`saleableArea-${index}`} type="number" step="0.01" defaultValue={Number(item.saleableArea || 0) || ''} style={tableInput} /></td><td style={{ ...cell, padding: 0 }}><input form="overview-products-form" name={`nonSaleableArea-${index}`} type="number" step="0.01" defaultValue={Number(item.nonSaleableArea || 0) || ''} style={tableInput} /></td><td style={{ ...cell, padding: 0 }}><input form="overview-products-form" name={`allocationWeight-${index}`} type="number" step="0.01" defaultValue={Number(item.allocationWeight || 0) || ''} style={{ ...tableInput, minWidth: 70 }} /></td><td style={{ ...cell, textAlign: 'center' }}><input form="overview-products-form" name={`isSaleable-${index}`} type="checkbox" defaultChecked={item.isSaleable} /></td><td style={{ ...cell, textAlign: 'center' }}><input form="overview-products-form" name={`participateAllocation-${index}`} type="checkbox" defaultChecked={item.participateAllocation} /></td><td style={{ ...cell, padding: 0 }}><input form="overview-products-form" name={`remark-${index}`} defaultValue={item.remark || ''} style={{ ...tableInput, minWidth: 150 }} /></td></tr>)}</tbody>
                </table>
              </div>
            </div>
            <form action={`/api/projects/${project.id}/products`} method="post" style={{ border: '1px solid #eef2f6', borderRadius: 10, padding: 12, background: '#fcfdff', display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <b>新增 / 更新业态</b>
              <p className="meta" style={{ margin: 0 }}>从后台业态模板中选择；已在本项目中的业态不会出现在下拉里，避免重复。</p>
              <label style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#475467' }}>业态名称<select name="name" required disabled={missingPresets.length === 0} defaultValue="" style={formField}><option value="" disabled>{missingPresets.length === 0 ? '模板业态均已选择' : '请选择未选业态'}</option>{missingPresets.map((group) => <optgroup key={group.category} label={group.category}>{group.names.map((name) => <option key={name} value={name}>{name}</option>)}</optgroup>)}</select></label>
              {addInput('buildingArea', '建筑面积㎡')}{addInput('capacityArea', '计容面积㎡')}{addInput('saleableArea', '可售面积㎡')}{addInput('nonSaleableArea', '不可售面积㎡')}{addInput('allocationWeight', '分摊权重')}
              <label style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 13, color: '#475467' }}>备注<input name="remark" placeholder="面积口径、适用范围" style={formField} /></label>
              ...
              <button className="btn btn-primary" disabled={missingPresets.length === 0}>添加到本项目</button>
              <Link href={`/projects/${project.id}/product-maintenance`} className="btn">业态增减维护</Link>
            </form>
          </div>
        </Block>

        <Block title="四、楼栋、车位与充电桩指标" note="充电桩数量在这里维护，不作为业态；车位收入通常按个数×单个车位含税单价，在车位收入测算页专项维护。">
          <FieldGrid project={project} fields={buildingIndicatorFields} />
          <div style={{ height: 12 }} />
          <FieldGrid project={project} fields={parkingIndicatorFields} />
        ...
        <Block title="五、工程量指标 - 5.1 总平、景观、道路、临设" note="用于围墙、出入口、临设、三通一平、道路总平、景观、室外管网等成本测算。"><FieldGrid project={project} fields={siteEngineeringFields} /></Block>
        <Block title="五、工程量指标 - 5.2 地下室、公区、示范区与配套" note="用于地下室、主楼地下室、人防、公区、大堂、售楼部、样板间、物业/社区用房等成本测算。"><FieldGrid project={project} fields={basementPublicFields} /></Block>
        <Block title="五、工程量指标 - 5.3 土建实体工程量" note="用于桩基、土方、防水、屋面、保温、外墙、门窗、栏杆等土建科目测算。"><FieldGrid project={project} fields={civilEngineeringFields} /></Block>
        <Block title="五、工程量指标 - 5.4 安装与设备数量" note="用于配电房、水泵房、消防水池等安装设备科目测算。"><FieldGrid project={project} fields={mepEquipmentFields} /></Block>
        <Block title="六、备注与口径说明" note="保留项目定位、面积口径、取数来源、特殊假设等，不参与计算。"><textarea form="overview-form" name="remark" defaultValue={project.remark || ''} placeholder="填写项目定位、测算边界、面积口径、取数说明等" style={{ width: '100%', minHeight: 90, border: '1px solid #d9e2ec', borderRadius: 8, padding: 10 }} /></Block>
