...
export default async function ProjectVersionsPage({ params, searchParams }: { params: { id: string }, searchParams?: Record<string, string | undefined> }) {
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1240 }}>
    <div className="page-header"><div><p className="eyebrow">版本管理</p><h1 className="title">{project.name}</h1><p className="subtitle">一个项目可以沉淀投拓、定位、方案、施工图、招采、动态成本、结算等不同阶段版本。当前版本会作为工作台和测算页面默认版本。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}`} className="btn btn-primary">返回工作台</Link><Link href="/projects" className="btn">项目列表</Link></div></div>
    ...
    <section className="card" style={{ marginBottom: 14 }}><h2>新增版本</h2><p className="meta">建议从当前版本复制，保留业态、科目规则、税率和已有测算口径，再在新阶段里深化。</p>
    ...
    <section className="card"><h2>已有版本</h2><div style={{ overflowX: 'auto', marginTop: 12 }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1080, fontSize: 13 }}><thead><tr>{['当前', '阶段', '版本名称', '状态', '业态', '科目规则', '成本明细', '含税成本', '操作'].map((head) => <th key={head} style={{ textAlign: 'left', padding: 10, borderBottom: '1px solid #eef2f6', color: '#667085' }}>{head}</th>)}</tr></thead><tbody>{versions.map((version) => {
      const cost = version.costs.reduce((sum, row) => sum + Number(row.taxInclusiveAmount || 0), 0);
      return <tr key={version.id} style={{ background: version.id === activeId ? '#f0fbfc' : '#fff' }}><td style={{ padding: 10, borderBottom: '1px solid #eef2f6', fontWeight: 900 }}>{version.id === activeId ? '当前' : '-'}</td><td style={{ padding: 10, borderBottom: '1px solid #eef2f6', fontWeight: 800 }}>{version.stage}</td><td style={{ padding: 10, borderBottom: '1px solid #eef2f6' }}>{version.name}</td><td style={{ padding: 10, borderBottom: '1px solid #eef2f6' }}>{version.status}{version.isLocked ? '｜已锁定' : ''}</td><td style={{ padding: 10, borderBottom: '1px solid #eef2f6' }}>{version.products.length}</td><td style={{ padding: 10, borderBottom: '1px solid #eef2f6' }}>{version.costRules.length}</td><td style={{ padding: 10, borderBottom: '1px solid #eef2f6' }}>{version.costs.length}</td><td style={{ padding: 10, borderBottom: '1px solid #eef2f6' }}>{fmt(cost)}</td><td style={{ padding: 10, borderBottom: '1px solid #eef2f6' }}><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>{version.id !== activeId ? <form action={`/api/projects/${project.id}/versions`} method="post"><input type="hidden" name="action" value="set-active" /><input type="hidden" name="versionId" value={version.id} /><button className="btn btn-primary">设为当前</button></form> : null}<form action={`/api/projects/${project.id}/versions`} method="post"><input type="hidden" name="action" value="copy" /><input type="hidden" name="sourceVersionId" value={version.id} /><input type="hidden" name="name" value={`${version.name}副本`} /><input type="hidden" name="stage" value={version.stage} /><button className="btn">复制</button><label style={{ display: 'flex', gap: 5, alignItems: 'center', fontSize: 12 }}><input name="copyCosts" type="checkbox" />含成本</label></form><form action={`/api/projects/${project.id}/versions`} method="post"><input type="hidden" name="action" value="delete" /><input type="hidden" name="versionId" value={version.id} /><button className="btn" style={{ color: '#c92a2a' }}>删除</button></form></div></td></tr>;
