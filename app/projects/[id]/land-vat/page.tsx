...
export default async function LandVatPage({ params }: { params: { id: string } }) {
  ...
  const objectMap = new Map<string, { product: any; revenueInclusive: number; revenueExclusive: number; outputVat: number; inputVat: number; landCost: number; devCost: number; saleManageFinance: number; costExclusive: number }>();
  ...
  const objectRows = Array.from(objectMap.values()).filter((item) => item.product.isSaleable).map((item) => {
    ...
    const lv = landVatSummary({
      revenueExclusive: item.revenueExclusive,
      outputVat: payableVat,
      landCost: item.landCost,
      devCost: item.devCost,
      saleManageFinance: item.saleManageFinance,
      surchargeRate
    });
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1380 }}>
    <div className="page-header"><div><p className="eyebrow">土地增值税测算表</p><h1 className="title">{project.name}</h1><p className="subtitle">与目标成本汇总表共用同一套收入、成本和税费口径；新增按住宅、商业、车位等成本对象测算。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/cost-allocation`} className="btn btn-primary">成本分摊</Link><Link href={`/projects/${project.id}/tax-details`} className="btn">税金明细</Link><Link href={`/projects/${project.id}/summary`} className="btn">目标成本汇总</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
