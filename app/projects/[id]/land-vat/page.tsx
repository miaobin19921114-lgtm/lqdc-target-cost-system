...
export default async function LandVatPage({ params }: { params: { id: string } }) {
  ...
  return <main className="page"><ProjectTopNav projectId={project.id} projectName={project.name} current="土地增值税清算测算表" /><div className="container" style={{ maxWidth: 1500 }}>
    <div className="page-header"><div><p className="eyebrow">土地增值税清算测算表</p><h1 className="title">{project.name}</h1><p className="subtitle">按清算对象分组，并优先读取模板中心的“土增税清算分摊规则”。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/product-maintenance`} className="btn btn-primary">业态维护/税务清算对象</Link><Link href={`/projects/${project.id}/tax-details`} className="btn">税费测算总表</Link><Link href={`/projects/${project.id}/summary`} className="btn">目标成本汇总</Link></div></div>
