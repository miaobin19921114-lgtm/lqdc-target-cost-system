...
export default async function VersionComparePage({ params, searchParams }: { params: { id: string }, searchParams?: Record<string, string | undefined> }) {
  ...
  return <main className="page"><div className="container" style={{ maxWidth: 1280 }}>
    <div className="page-header"><div><p className="eyebrow">版本对比</p><h1 className="title">{project.name}</h1><p className="subtitle">对比不同测算阶段版本的收入、成本、利润和基础数据规模。</p></div><div className="actions" style={{ marginTop: 0 }}><Link href={`/projects/${project.id}/versions`} className="btn btn-primary">返回版本中心</Link><Link href={`/projects/${project.id}/summary`} className="btn">目标成本汇总</Link><Link href={`/projects/${project.id}`} className="btn">返回工作台</Link></div></div>
